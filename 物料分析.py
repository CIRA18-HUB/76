    ...
    with col1:
        ...
        if len(segment_counts) > 0:
            ...
            for segment in segment_counts['客户价值分层']:
                ...
                disp_text = ", ".join([d for d in distributors[:3]])
            ...
            if '所属区域' in filtered_distributor.columns and len(
                ...
                try:
                    ...
                    custom_data = np.column_stack((
                        heatmap_long['平均物料多样性'].fillna(0).astype(float),
                        heatmap_long['平均物料产出比'].fillna(0).astype(float)
                    ))
                ...
                # 更新布局
                fig2.update_layout(
                    height=550,  # 与第一个图表高度匹配
                    margin=dict(l=40, r=40, t=50, b=160),  # 增加底部边距
                    coloraxis_colorbar=dict(
                        title="使用占比 (%)",
                        ticksuffix="%"
                    ),
...
def create_expense_ratio_analysis(filtered_distributor):
    ...
    if filtered_distributor is not None and len(filtered_distributor) > 0:
        ...
        if len(valid_data) > 0:
            ...
            # 截断经销商名称，使其不会太长
            plot_data['经销商名称显示'] = plot_data['经销商名称'].apply(lambda x: x[:15] + '...' if len(x) > 15 else x)
            ...
            fig2.update_traces(
                ...
                customdata=np.column_stack((
                    plot_data['客户价值分层'].fillna('未分类'),  # 添加fillna防止空值
                    plot_data['物料总成本'].fillna(0),
                    plot_data['销售总额'].fillna(0)
                ))
            )
            ...
            fig2.update_layout(
                ...
                height=max(800, len(plot_data) * 30),  # 动态调整高度，确保每个条形有足够空间
                xaxis=dict(
                    title="物料费比 (%)",
                    showgrid=True,
                    gridcolor='rgba(224, 228, 234, 0.6)',
                    ticksuffix="%",
                    range=[0, max(plot_data['物料销售比率']) * 1.15]  # 增加右侧边距
                ),
                yaxis=dict(
                    title="经销商",
                    showgrid=False,
                ...
                margin=dict(l=180, r=60, t=60, b=40),  # 增加左侧边距到180px
                showlegend=False,
                plot_bgcolor='white',
                paper_bgcolor='white'
            )
