...
<p>这条线显示了物料投入产出比的变化。该指标仅计入物料成本，不含赠品等其他促销成本。红色虚线(物料投入产出比=1)表示盈亏平衡，紫色虚线(物料投入产出比=2)表示优秀水平。线在红线以下表示亏损，越高越好。悬停可查看详细数据，包括物料成本和环比变化。</p>
            ...
            with col1:
                ...
                if '销售人员' in filtered_distributor.columns and len(filtered_distributor) > 0:
                    ...
                    # 尝试从原始数据查找销售人员
                    unique_salespersons = []
                    if '申请人' in material_data.columns:  # 修改这里以检查"申请人"列
                    ...
                    elif '申请人' in sales_data.columns:  # 也检查销售数据中的"申请人"列
                ...
                if '客户价值分层' in filtered_distributor.columns and '销售人员' in filtered_distributor.columns:
                    ...
                    if len(salesperson_segments) > 0:
                        ...
                        fig.update_layout(
                            ...
                            xaxis_title='',
                            yaxis_title='经销商数量',
                            margin=dict(l=20, r=20, t=40, b=80),  # 增加底部边距避免遮挡
                            paper_bgcolor='white',
                            plot_bgcolor='white',
                            font=dict(
                                family="PingFang SC, Helvetica Neue, Arial, sans-serif",
                    ...
                    # 检查申请人列作为替代
                    if '客户价值分层' in filtered_distributor.columns and '申请人' in filtered_distributor.columns:
                        st.warning(
                            "检测到'申请人'列，但未将其映射为'销售人员'。请在数据处理阶段将'申请人'列重命名为'销售人员'。")
                    else:
        ...
        with tab2:
            ...
            if len(material_sales_relation) > 0:
                ...
                # 设置气泡大小 - 降低整体大小，减少重叠
                # 使用ROI的对数值来缩放，避免极值造成太大的气泡
                size_values = np.log1p(material_sales_relation['ROI'].clip(0.1, 10)) * 10
                    ...
                    if len(segment_data) > 0:
                        ...
                        fig.add_trace(go.Scatter(
                            ...
                            marker=dict(
                                size=segment_size,
                                color=color,
                                opacity=0.75,  # 降低不透明度，减轻重叠感
                                line=dict(width=1, color='white'),
                                symbol='circle',
                                sizemode='diameter',
                                sizeref=0.4,  # 增大此值可以缩小所有气泡
                            ),
                            name=segment,
                            ...
                            customdata=np.column_stack((
                                ...
                                segment_data['所属区域'] if '所属区域' in segment_data.columns else ['未知'] * len(
                                    segment_data),
                                segment_data['省份'] if '省份' in segment_data.columns else ['未知'] * len(
                                    segment_data),
                                segment_data['销售人员'] if '销售人员' in segment_data.columns else ['未知'] * len(
                                    segment_data),
                                segment_data['月份名'] if '月份名' in segment_data.columns else ['未知'] * len(
                                    segment_data),
                                segment_data['客户代码']
                            ))
                        ))
            ...
            <div class="chart-explanation">
                ...
                背景区域划分了不同ROI的区域：低效区(ROI<1)、良好区(1≤ROI<2)和优秀区(ROI≥2)。悬停在点上可查看更多经销商详情。</p>
            </div>
            ''', unsafe_allow_html=True)
            ...
            with col3:
                st.markdown('<div style="margin-top: 5px;">选择显示物料ROI数据的方式和数量</div>',
                            unsafe_allow_html=True)
            ...
            if len(material_roi) > 0:
                ...
                fig = px.bar(
                    ...
                    color='物料类别',
                    text='物料ROI',
                    title=f"{'TOP' if show_option == '最高ROI' else '最低'} {top_n} 物料投入产出比分析",
                    height=500,
                ...
                fig.update_traces(
                    ...
                    marker=dict(line=dict(width=0.8, color='white')),
                    hovertemplate='<b>%{x}</b><br>平均ROI: %{y:.2f}<br>物料类别: %{marker.color}<br>使用客户数: %{customdata[0]}<br>使用总量: %{customdata[1]}<br>产品代码: %{customdata[2]}',
            ...
            with col1:
                ...
                if len(distributor_cost_ratio) > 0:
                    ...
                    fig.add_trace(go.Bar(
                        ...
                        hovertemplate='<b>%{y}</b><br>物料费比: %{x:.2f}%<br>物料成本: ¥%{customdata[0]:,.2f}<br>销售总额: ¥%{customdata[1]:,.2f}<br>客户价值分层: %{customdata[2]}<br>客户代码: %{customdata[3]}',
                        customdata=top_distributors[['物料总成本', '销售总额', '客户价值分层', '客户代码']]
                    ))
                ...
                <div class="chart-explanation">
                    <div class="chart-explanation-title">图表解读：</div>
                    <p>此图展示了TOP 10经销商的物料费比(物料成本占销售额的百分比)，已按销售额排序。
