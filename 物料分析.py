...
def load_data(sample_data=False):
    ...
    if sample_data:
        ...
        try:
            ...
            # 数据验证和列名修正
            # 检查物料数量列是否需要重命名
            if '物料数量' in material_data.columns and '求和项:数量' not in material_data.columns:
                material_data['求和项:数量'] = material_data['物料数量']
                st.info("已将'物料数量'列重命名为'求和项:数量'以便于处理")
...
def validate_data_columns(material_data, sales_data, material_price):
    ...
    # 检查物料数据
    material_required_cols = ['物料代码', '经销商名称', '发运月份']  # 移除求和项:数量，因为我们已在load_data中处理
    ...
    # 确保求和项:数量列存在
    if '求和项:数量' not in material_data.columns:
        if '物料数量' in material_data.columns:
            material_data['求和项:数量'] = material_data['物料数量']
            st.info("已将'物料数量'列映射为'求和项:数量'")
...
def create_distributor_analysis_tab(filtered_distributor, material_data, sales_data):
    ...
    with col2:
        ...
        if '物料费率' in filtered_distributor.columns or '物料销售比率' in filtered_distributor.columns:
            ...
            # 创建物料费率区间 - 修复：使用百分比值而非小数
            if fee_rate_col == '物料销售比率':
                # 物料销售比率已经是百分比值（0-100）
