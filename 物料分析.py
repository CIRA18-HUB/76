...
def process_data(material_data, sales_data, material_price):
    ...
    if '物料成本' not in material_data.columns:
        ...
        if '单价（元）' in material_data.columns:
            ...
            # 计算物料总成本 - 修复：使用物料数量而非求和项:数量
            if '物料数量' in material_data.columns:
                material_data['物料成本'] = material_data['物料数量'] * material_data['单价（元）']
            elif '求和项:数量' in material_data.columns:
                material_data['物料成本'] = material_data['求和项:数量'] * material_data['单价（元）']
            else:
                # 如果都不存在，创建一个默认值
                st.warning("物料数据中未找到'物料数量'或'求和项:数量'列，使用默认值100计算物料成本")
                material_data['物料成本'] = 100 * material_data['单价（元）']
        else:
            # 如果合并失败，创建一个默认的物料成本列
            st.warning("未能成功合并物料单价数据，使用默认值计算物料成本")
            if '物料数量' in material_data.columns:
                material_data['物料成本'] = material_data['物料数量'] * 100  # 使用默认单价100
            elif '求和项:数量' in material_data.columns:
                material_data['物料成本'] = material_data['求和项:数量'] * 100  # 使用默认单价100
            else:
                material_data['物料成本'] = 10000  # 完全默认值
...
def main():
    ...
    # 这里是应用中唯一的st.set_page_config()调用，必须是第一个Streamlit命令
    st.set_page_config(
...
# 应用入口点：确保main()只被调用一次，并且是脚本的最后一个命令
