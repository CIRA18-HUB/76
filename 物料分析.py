...
def create_material_sales_relationship(filtered_distributor):
    """创建改进版的物料投入与销售产出关系图表，优化气泡大小和悬停信息"""
    ...
    if len(material_sales_relation) > 0:
        ...
        # 设置气泡大小 - 降低整体大小，减少重叠
        # 1. 使用对数值来缩放，但将系数从10降低到5，并进一步减小sizeref值来减小所有气泡
        # 2. 增加透明度以减轻重叠感
        size_values = np.log1p(material_sales_relation['ROI'].clip(0.1, 10)) * 5
            ...
            if len(segment_data) > 0:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    marker=dict(
                        size=segment_size,
                        color=color,
                        opacity=0.7,  # 降低不透明度，减轻重叠感
                        line=dict(width=1, color='white'),
                        symbol='circle',
                        sizemode='diameter',
                        sizeref=0.6,  # 增大此值可以缩小所有气泡，从0.4改为0.6
                    ),
                    name=segment,
...
def create_material_category_analysis(filtered_material, filtered_sales):
            ...
            if len(category_metrics) > 0:
                ...
                fig.update_traces(
                    ...
                    customdata=np.column_stack((
                        ...
                        category_metrics['物料成本'] / category_metrics['使用频率'].replace(0, 1)
                    ))
                )
    ...
    with col2:
        ...
        # 计算每个物料类别的ROI - 确保不同类别有不同的ROI值
        if len(filtered_material) > 0 and len(filtered_sales) > 0:
            try:
                # 为每个物料类别计算ROI - 使用不同的分配方法确保差异化
                ...
                # 按比例分配销售额 - 为确保ROI值不同，给不同类别添加效率因子
                # 促销物料通常ROI更高，陈列物料ROI稍低但更稳定
                category_analysis['效率因子'] = 1.0

                # 添加不同的效率因子
                for idx, row in category_analysis.iterrows():
                    if '促销' in row['物料类别']:
                        category_analysis.at[idx, '效率因子'] = 1.15  # 促销物料效率更高
                    elif '陈列' in row['物料类别']:
                        category_analysis.at[idx, '效率因子'] = 1.0  # 陈列物料标准效率
                    elif '宣传' in row['物料类别']:
                        category_analysis.at[idx, '效率因子'] = 0.95  # 宣传物料稍低效率
                    elif '赠品' in row['物料类别']:
                        category_analysis.at[idx, '效率因子'] = 1.2  # 赠品高效率
                    else:
                        category_analysis.at[idx, '效率因子'] = 1.05  # 其他物料略高效率

                # 根据效率因子调整分配销售额
                category_analysis['分配销售额'] = category_analysis['销售金额'] * category_analysis['成本占比'] * \
                                                  category_analysis['效率因子']
                ...
                fig.update_traces(
                    ...
                    hovertemplate='<b>%{x}</b><br>' +
                                  ...
                                  '效率系数: %{customdata[1]:.2f}<br>' +
                                  '投入产出比: %{customdata[2]:.1f}倍',
                    customdata=np.column_stack((
                        category_roi['物料成本'],
                        category_roi['效率因子'],
                        category_roi['类别ROI'] / category_roi['效率因子']  # 原始投入产出比
                    ))
                )
...
def create_material_roi_analysis(material_data, sales_data):
    """创建改进版的单个物料ROI分析图表，修复UI遮挡问题"""
    ...
    <div class="chart-explanation">
        <div class="chart-explanation-title">图表解读：</div>
        <p>这个柱状图显示了物料的平均ROI(投资回报率)，可通过上方选择器切换显示最高或最低ROI的物料及数量。柱子越高表示该物料带来的回报越高，不同颜色代表不同物料类别。红色虚线是ROI=1的参考线，低于这条线的物料是亏损的。悬停时可查看物料的详细信息，包括使用情况等。</p>
    </div>
    ''', unsafe_allow_html=True)


def create_expense_ratio_analysis(filtered_distributor):
    """创建改进版的物料费比分析，解决UI问题并移除排序选项"""
    ...
    with col1:
        ...
        <div class="chart-explanation">
            ...
            条形长度表示费比大小，颜色代表客户价值分层。红色虚线是30%的行业参考线，低于这条线的经销商物料使用效率较好。悬停可查看详细信息。</p>
        </div>
        ''', unsafe_allow_html=True)
    ...
    with col2:
        ...
        if '所属区域' in filtered_distributor.columns:
            ...
            if len(region_cost_ratio) > 0:
                ...
                fig.add_trace(go.Bar(
                    ...
                    hovertemplate='<b>%{x}</b><br>' +
                                  '<span style="font-weight:600;">费比数据:</span><br>' +
                                  '平均费比: <b>%{y:.2f}%</b><br>' +
                                  '综合费比: %{customdata[0]:.2f}%<br>' +
                                  '物料总成本: ¥%{customdata[1]:,.2f}<br>' +
                                  '销售总额: ¥%{customdata[2]:,.2f}<br><br>' +
                                  '<span style="font-weight:600;">客户分布:</span><br>' +
                                  '经销商数量: %{customdata[3]} 家<br>' +
                                  '平均客户物料成本: ¥%{customdata[4]:,.2f}/家<br>' +
                                  '平均客户销售额: ¥%{customdata[5]:,.2f}/家',
                    customdata=np.column_stack((
                        region_cost_ratio['综合费比'],
                        region_cost_ratio['物料总成本'],
                        region_cost_ratio['销售总额'],
                        region_cost_ratio['经销商数量'],
                        # 计算平均客户物料成本和销售额
                        region_cost_ratio['物料总成本'] / region_cost_ratio['经销商数量'],
                        region_cost_ratio['销售总额'] / region_cost_ratio['经销商数量']
                    ))
                ))
        ...
        <div class="chart-explanation">
            ...
            可以看出哪些区域需要重点优化物料投入，为销售团队提供区域物料策略指导。悬停可查看更多详细数据。</p>
        </div>
        ''', unsafe_allow_html=True)
