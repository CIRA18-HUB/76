...
def main():
        ...
        with tab1:
            ...
            # 物料成本卡 - 增强版
            material_change = 0  # 这里可以计算环比变化
            trend_icon = "↑" if material_change >= 0 else "↓"
            trend_class = "success-value" if material_change >= 0 else "danger-value"
            ...
            st.markdown(f'''
                <div class="feishu-metric-card">
                    ...
                    <div class="subtext">平均: ¥{(total_material_cost / total_distributors if total_distributors > 0 else 0):,.2f}/经销商 
                    <span class="{trend_class}">{trend_icon} {abs(material_change):.1f}%</span></div>
                </div>
            ''', unsafe_allow_html=True)

            # 销售总额卡 - 增强版
            sales_change = 0  # 这里可以计算环比变化
            trend_icon = "↑" if sales_change >= 0 else "↓"
            trend_class = "success-value" if sales_change >= 0 else "danger-value"
            ...
            st.markdown(f'''
                <div class="feishu-metric-card">
                    ...
                    <div class="subtext">平均: ¥{(total_sales / total_distributors if total_distributors > 0 else 0):,.2f}/经销商
                    <span class="{trend_class}">{trend_icon} {abs(sales_change):.1f}%</span></div>
                </div>
            ''', unsafe_allow_html=True)

            # ROI卡 - 增强版
            roi_change = 0  # 这里可以计算环比变化
            trend_icon = "↑" if roi_change >= 0 else "↓"
            trend_class = "success-value" if roi_change >= 0 else "danger-value"
            ...
            st.markdown(f'''
                <div class="feishu-metric-card">
                    ...
                    <div class="subtext">销售总额 ÷ 物料总成本 
                    <span class="{trend_class}">{trend_icon} {abs(roi_change):.1f}%</span></div>
                </div>
            ''', unsafe_allow_html=True)

            # 物料销售比率卡 - 增强版
            ratio_change = 0  # 这里可以计算环比变化
            trend_icon = "↑" if ratio_change <= 0 else "↓"  # 注意这里相反，因为比率越低越好
            trend_class = "success-value" if ratio_change <= 0 else "danger-value"
            ...
            st.markdown(f'''
                <div class="feishu-metric-card">
                    ...
                    <div class="subtext">物料总成本 ÷ 销售总额 × 100% 
                    <span class="{trend_class}">{trend_icon} {abs(ratio_change):.1f}%</span></div>
                </div>
            ''', unsafe_allow_html=True)
            ...
            <div class="chart-explanation">
                ...
                <p>上面四个指标卡显示了本期业绩情况。物料总成本是花在物料上的钱，销售总额是赚到的钱。ROI值大于1就是赚钱了，大于2是非常好的效果。物料销售比率越低越好，意味着花少量的钱带来了更多的销售。百分比变化表示与上期相比的增减趋势。</p>
            </div>
            ''', unsafe_allow_html=True)
            ...
            with col2:
                ...
                # 添加物料销售比率计算
                monthly_roi['物料销售比率'] = (monthly_roi['物料成本'] / monthly_roi['销售总额'] * 100).round(
                    2) if '销售总额' in monthly_roi.columns else 0
                ...
                <div class="chart-explanation">
                    <div class="chart-explanation-title">图表解读：</div>
                    <p>这条线显示了投资回报率(ROI)的变化。红色虚线(ROI=1)表示盈亏平衡，紫色虚线(ROI=2)表示优秀水平。线在红线以下表示亏损，越高越好。悬停可查看详细数据，包括物料成本和环比变化。</p>
                </div>
                ''', unsafe_allow_html=True)

            # 新增：物料效率矩阵（不重复的新图表）
            st.markdown('<div class="feishu-chart-title" style="margin-top: 20px;">物料投入产出效率矩阵</div>',
                        unsafe_allow_html=True)

            st.markdown('<div class="feishu-chart-container">', unsafe_allow_html=True)

            # 按月份和物料类型分析
            if '物料类别' in filtered_material.columns and len(filtered_material) > 0:
                # 计算每个月每种物料类别的投入与产出
                monthly_material_category = filtered_material.groupby(['月份名', '物料类别'])[
                    '物料成本'].sum().reset_index()
                monthly_sales_total = filtered_sales.groupby('月份名')['销售金额'].sum().reset_index()

                # 合并数据
                efficiency_matrix = pd.merge(monthly_material_category, monthly_sales_total, on='月份名')

                # 计算每月每类物料占总成本的比例
                monthly_material_total = filtered_material.groupby('月份名')['物料成本'].sum().reset_index()
                monthly_material_total.columns = ['月份名', '月度物料总成本']
                efficiency_matrix = pd.merge(efficiency_matrix, monthly_material_total, on='月份名')

                # 计算比例和效率指标
                efficiency_matrix['成本占比'] = (
                            efficiency_matrix['物料成本'] / efficiency_matrix['月度物料总成本'] * 100).round(2)
                efficiency_matrix['分配销售额'] = efficiency_matrix['销售金额'] * (
                            efficiency_matrix['物料成本'] / efficiency_matrix['月度物料总成本'])
                efficiency_matrix['效率指数'] = (efficiency_matrix['分配销售额'] / efficiency_matrix['物料成本']).round(
                    2)

                # 添加月份序号用于排序
                efficiency_matrix['月份序号'] = pd.to_datetime(efficiency_matrix['月份名']).dt.strftime('%Y%m').astype(
                    int)

                # 只保留最近6个月的数据（避免图表过于复杂）
                recent_months = efficiency_matrix.sort_values('月份序号', ascending=False)['月份名'].unique()[:6]
                efficiency_matrix = efficiency_matrix[efficiency_matrix['月份名'].isin(recent_months)]

                # 按月份排序
                efficiency_matrix = efficiency_matrix.sort_values('月份序号')
                ...
                fig = px.density_heatmap(
                    ...
                    title='物料类别月度效率矩阵分析',
                    labels={'效率指数': '效率指数 (ROI)'},
                    range_color=[0, max(3, efficiency_matrix['效率指数'].max())],
                    text_auto='.2f',
                    height=500
                )
                ...
                fig.update_traces(
                    hovertemplate='<b>%{y} - %{x}</b><br>' +
                                  ...
                                  '成本占比: %{customdata[1]:.2f}%<br>' +
                                  '分配销售额: ¥%{customdata[2]:,.2f}',
                    customdata=np.column_stack((
                        efficiency_matrix['物料成本'],
                        efficiency_matrix['成本占比'],
                        efficiency_matrix['分配销售额']
                    ))
                )
                ...
                fig.update_layout(
                    ...
                    coloraxis_colorbar=dict(
                        title='效率指数',
                        tickvals=[0, 1, 2, 3],
                        ticktext=['0 (亏损)', '1 (盈亏平衡)', '2 (良好)', '3+ (优秀)']
                    ),
                    paper_bgcolor='white',
                    plot_bgcolor='white',
                    font=dict(
            ...
            <div class="chart-explanation">
                <div class="chart-explanation-title">图表解读：</div>
                <p>这个热力图展示了不同月份中各物料类别的投入产出效率。颜色越绿表示效率越高(ROI越高)，红色表示效率低。通过这个矩阵可以直观地看出哪些物料在哪些月份表现最佳，以及效率趋势的变化。悬停可查看详细数据，包括成本占比和分配销售额。</p>
            </div>
            ''', unsafe_allow_html=True)

            # 新增：销售人员物料使用效率对比（完全新的图表）
            if '销售人员' in filtered_distributor.columns:
                ...
                with col1:
                    ...
                    min_ratio = salesperson_metrics['物料销售比率'].min()
                    ...
                    # 只显示TOP 10
                    top_salespeople = salesperson_metrics.head(10)
                    ...
                    # 添加效率得分柱状图
                    fig.add_trace(go.Bar(
                        y=top_salespeople['销售人员'],
                        x=top_salespeople['效率得分'],
                        orientation='h',
                        name='物料利用效率得分',
                        marker=dict(
                            color=top_salespeople['效率得分'].apply(
                                lambda
                                    x: '#0FC86F' if x >= 75 else '#2B5AED' if x >= 60 else '#FFAA00' if x >= 40 else '#F53F3F'
                            ),
                            line=dict(width=0.5, color='white')
                        ),
                        text=top_salespeople['效率得分'].apply(lambda x: f"{x:.1f}"),
                        textposition='auto',
                        ...
                        customdata=np.column_stack((
                            top_salespeople['ROI'],
                            top_salespeople['物料销售比率'],
                            top_salespeople['经销商数量'],
                            top_salespeople['人均销售额']
                        ))
                    ))
                    ...
                    fig.add_shape(
                        type="rect",
                        x0=0, x1=40,
                        y0=-0.5, y1=len(top_salespeople) - 0.5,
                    ...
                    # 添加效率等级注释
                    fig.add_annotation(
                        x=20, y=len(top_salespeople) - 0.5,
                        text="不足",
                        showarrow=False,
                        font=dict(size=10, color="#F53F3F")
                    )
                    ...
                    fig.add_annotation(
                        x=50, y=len(top_salespeople) - 0.5,
                        text="一般",
                        showarrow=False,
                    ...
                    fig.add_annotation(
                        x=67.5, y=len(top_salespeople) - 0.5,
                        text="良好",
                        showarrow=False,
                    ...
                    fig.add_annotation(
                        x=87.5, y=len(top_salespeople) - 0.5,
                        text="优秀",
                        showarrow=False,
                    ...
                    <div class="chart-explanation">
                        <div class="chart-explanation-title">图表解读：</div>
                        <p>此图展示了销售人员的物料利用效率得分，满分100分。得分基于ROI(占60%)和物料销售比率(占40%)计算。得分越高表示该销售人员越善于利用物料资源创造销售额。背景色区分了不同效率等级：绿色(优秀≥75)、蓝色(良好≥60)、黄色(一般≥40)和红色(不足<40)。</p>
                    </div>
                    ''', unsafe_allow_html=True)
                ...
                with col2:
                    ...
                    # 计算销售人员的经销商客户结构
                    if '客户价值分层' in filtered_distributor.columns:
                        # 计算每个销售人员的客户结构
                        salesperson_segments = filtered_distributor.groupby(['销售人员', '客户价值分层']).agg({
                            '客户代码': 'nunique'
                        }).reset_index()

                        salesperson_segments.columns = ['销售人员', '客户价值分层', '经销商数量']

                        # 计算总客户数，用于计算占比
                        salesperson_totals = salesperson_segments.groupby('销售人员')['经销商数量'].sum().reset_index()
                        salesperson_totals.columns = ['销售人员', '总经销商数']

                        # 合并数据
                        salesperson_segments = pd.merge(salesperson_segments, salesperson_totals, on='销售人员')
                        salesperson_segments['占比'] = (salesperson_segments['经销商数量'] / salesperson_segments[
                            '总经销商数'] * 100).round(2)

                        # 只保留TOP销售人员的数据
                        top_salespeople_list = top_salespeople['销售人员'].tolist()
                        salesperson_segments = salesperson_segments[
                            salesperson_segments['销售人员'].isin(top_salespeople_list)]

                        # 创建分层条形图
                        fig = px.bar(
                            salesperson_segments,
                            x='销售人员',
                            y='经销商数量',
                            color='客户价值分层',
                            color_discrete_map={
                        ...
                        fig.update_layout(
                            ...
                            xaxis=dict(
                                tickangle=-45,
                                categoryorder='total descending'  # 按总量排序
                            ),
                            yaxis=dict(
                                showgrid=True,
                                gridcolor='#E0E4EA'
                    ...
                    <div class="chart-explanation">
                        <div class="chart-explanation-title">图表解读：</div>
                        <p>此图展示了TOP销售人员的客户结构。每个柱子代表一位销售人员，不同颜色代表不同类型的客户：绿色(高价值客户)、蓝色(成长型客户)、黄色(稳定型客户)和红色(低效型客户)。柱内百分比显示各类客户的占比。高效销售人员通常拥有更多的高价值和成长型客户。</p>
                    </div>
                    ''', unsafe_allow_html=True)
            else:
                st.info("暂无销售人员数据，无法生成销售人员效率分析。")

            # 新增：物料-销售季节性匹配分析（全新图表）
            st.markdown('<div class="feishu-chart-title" style="margin-top: 20px;">物料-销售季节性匹配分析</div>',
                        unsafe_allow_html=True)
            ...
            # 将月份转为季度
            monthly_data['季度'] = pd.to_datetime(monthly_data['月份名']).dt.quarter
            quarterly_data = monthly_data.groupby('季度').agg({
                '物料成本': 'sum',
                '销售金额': 'sum'
            }).reset_index()

            # 计算季度百分比
            quarterly_data['物料成本占比'] = (
                        quarterly_data['物料成本'] / quarterly_data['物料成本'].sum() * 100).round(2)
            quarterly_data['销售额占比'] = (quarterly_data['销售金额'] / quarterly_data['销售金额'].sum() * 100).round(
                2)
            quarterly_data['占比差异'] = (quarterly_data['销售额占比'] - quarterly_data['物料成本占比']).round(2)
            quarterly_data['季度名称'] = quarterly_data['季度'].apply(lambda x: f"Q{x}")
            ...
            fig.add_trace(go.Scatterpolar(
                r=quarterly_data['物料成本占比'],
                theta=quarterly_data['季度名称'],
                fill='toself',
                name='物料成本占比',
                line_color='#2B5AED',
                fillcolor='rgba(43, 90, 237, 0.2)',
                hovertemplate='%{theta}季度<br>' +
                              '物料成本占比: <b>%{r:.2f}%</b><br>' +
                              '物料成本: ¥%{customdata[0]:,.2f}',
                customdata=np.column_stack((
                    quarterly_data['物料成本'],
                ))
            ))
            ...
            # 添加销售额占比
            fig.add_trace(go.Scatterpolar(
                r=quarterly_data['销售额占比'],
                theta=quarterly_data['季度名称'],
                fill='toself',
                name='销售额占比',
                line_color='#0FC86F',
                fillcolor='rgba(15, 200, 111, 0.2)',
                hovertemplate='%{theta}季度<br>' +
                              '销售额占比: <b>%{r:.2f}%</b><br>' +
                              '销售金额: ¥%{customdata[0]:,.2f}<br>' +
                              '与物料成本占比差异: %{customdata[1]:+.2f}%',
                customdata=np.column_stack((
                    quarterly_data['销售金额'],
                    quarterly_data['占比差异']
                ))
            ...
            fig.update_layout(
                polar=dict(
                    radialaxis=dict(
                        visible=True,
                        range=[0, max(
                            quarterly_data['物料成本占比'].max(),
                            quarterly_data['销售额占比'].max()
                        ) * 1.2]
                    )
                ),
                title='季度物料投入与销售产出匹配度分析',
                height=500,
            ...
            fig2.add_trace(go.Bar(
                x=quarterly_data['季度名称'],
                y=quarterly_data['占比差异'],
                marker_color=quarterly_data['占比差异'].apply(
                    lambda x: '#0FC86F' if x > 5 else '#FFAA00' if x > 0 else '#F53F3F'
                ),
                text=quarterly_data['占比差异'].apply(lambda x: f"{x:+.2f}%"),
                textposition='auto',
                hovertemplate='%{x}季度<br>' +
                              '占比差异: <b>%{y:+.2f}%</b><br>' +
                              '销售额占比: %{customdata[0]:.2f}%<br>' +
                              '物料成本占比: %{customdata[1]:.2f}%',
                customdata=np.column_stack((
                    quarterly_data['销售额占比'],
                    quarterly_data['物料成本占比']
                ))
            ))
            ...
            <div class="chart-explanation">
                <div class="chart-explanation-title">图表解读：</div>
                <p>左侧雷达图展示了各季度物料成本(蓝色)和销售额(绿色)的分布占比。理想情况下，两者应该匹配，表示物料投入与销售产出季节性匹配良好。右侧柱状图展示了销售占比减物料占比的差异：绿色正值表示该季度物料利用效率高，红色负值表示效率低，需要调整物料投放策略。</p>
            </div>
            ''', unsafe_allow_html=True)
        # ======= 经销商分析标签页 =======
        with tab3:
        ...
        with tab2:
            ...
            if len(material_sales_relation) > 0:
                ...
                # 设置气泡大小 - 显著减小气泡大小，避免重叠
                size_values = material_sales_relation['ROI'].clip(0.1, 5) * 1.2  # 显著减小系数
                    ...
                    if len(segment_data) > 0:
                        ...
                        fig.add_trace(go.Scatter(
                            ...
                            marker=dict(
                                ...
                                symbol='circle',
                                sizemode='diameter',
                                sizeref=0.35,  # 增加sizeref值以进一步减小气泡
                            ),
                            name=segment,
                            ...
                            hovertemplate='<b>%{hovertext}</b><br>' +
                                          ...
                                          '销售总额: <b>¥%{y:,.2f}</b><br>' +
                                          '物料成本: <b>¥%{x:,.2f}</b><br>' +
                                          'ROI: <b>%{customdata[0]:.2f}</b><br>' +
                                          '物料销售比率: <b>%{customdata[1]:.2f}%</b><br>' +
                                          '客户价值分层: <b>%{customdata[2]}</b><br>' +
                                          '物料多样性: <b>%{customdata[3]}</b> 种<br>' +
                                          '所属区域: <b>%{customdata[4]}</b><br>' +
                                          '省份: <b>%{customdata[5]}</b><br>' +
                                          '销售人员: <b>%{customdata[6]}</b>',
                            customdata=np.column_stack((
                                segment_data['ROI'],
                                segment_data['物料销售比率'],
                                segment_data['客户价值分层'],
                                segment_data['物料多样性'] if '物料多样性' in segment_data.columns else np.zeros(
                                    len(segment_data)),
                                segment_data['所属区域'] if '所属区域' in segment_data.columns else np.array(
                                    ['未知'] * len(segment_data)),
                                segment_data['省份'] if '省份' in segment_data.columns else np.array(
                                    ['未知'] * len(segment_data)),
                                segment_data['销售人员'] if '销售人员' in segment_data.columns else np.array(
                                    ['未知'] * len(segment_data))
                            ))
                        ))
            ...
            <div class="chart-explanation">
                ...
                区域标识划分了不同ROI的区域：低效区(ROI<1)、良好区(1≤ROI<2)和优秀区(ROI≥2)。</p>
            </div>
            ''', unsafe_allow_html=True)
                ...
                if '物料类别' in filtered_material.columns and '物料成本' in filtered_material.columns:
                    ...
                    if len(category_metrics) > 0:
                        ...
                        fig.update_traces(
                            ...
                            hovertemplate='<b>%{x}</b><br>物料成本: ¥%{y:,.2f}<br>占比: %{text}%<br>物料种类: %{customdata[0]}种',
                            customdata=category_metrics[['使用频率']]
                        )
                        ...
                        fig.update_layout(
                            ...
                            xaxis_title="物料类别",
                            yaxis_title="物料成本 (元)",
                            margin=dict(l=40, r=40, t=50, b=80),  # 增加底部margin避免遮挡
            ...
            with col2:
                ...
                if len(filtered_material) > 0 and len(filtered_sales) > 0:
                    try:
                        ...
                        # 确保类别不重复（修复类别重复的问题）
                        category_roi = category_roi.drop_duplicates(subset=['物料类别'])
                        ...
                        fig.update_traces(
                            ...
                            textfont=dict(size=12, color="#333333", family="PingFang SC"),
                            marker=dict(line=dict(width=0.8, color='white')),
                            hovertemplate='<b>%{x}</b><br>平均ROI: %{y:.2f}<br>物料成本: ¥%{customdata[0]:,.2f}',
                            customdata=category_roi[['物料成本']]
                        )
            ...
            with col1:
                top_n = st.selectbox("显示数量", [10, 15, 20, 30], index=1)
            with col2:
                show_option = st.selectbox("显示方式", ["最高ROI", "最低ROI"], index=0)
            with col3:
                st.markdown('选择显示物料ROI数据的方式和数量', unsafe_allow_html=True)
            ...
            if len(material_roi) > 0:
                # 根据显示选项筛选数据
                if show_option == "最高ROI":
                    filtered_material_roi = material_roi.sort_values('物料ROI', ascending=False).head(top_n)
                else:  # "最低ROI"
                    filtered_material_roi = material_roi.sort_values('物料ROI').head(top_n)
                ...
                fig = px.bar(
                    ...
                    color='物料类别',
                    text='物料ROI',
                    title=f"{'TOP' if show_option == '最高ROI' else '最低'} {top_n} 物料ROI分析",
                    height=500,
                    color_discrete_map=color_mapping,
                    labels={"产品名称": "物料名称", "物料ROI": "平均ROI", "物料类别": "物料类别"}
                )
                ...
                fig.update_traces(
                    ...
                    hovertemplate='<b>%{x}</b><br>' +
                                  '<b>详细信息:</b><br>' +
                                  '平均ROI: <b>%{y:.2f}</b><br>' +
                                  '物料类别: <b>%{marker.color}</b><br>' +
                                  '使用客户数: <b>%{customdata[0]}</b><br>' +
                                  '使用总量: <b>%{customdata[1]} 箱</b><br>' +
                                  '产品代码: <b>%{customdata[2]}</b>',
                    customdata=filtered_material_roi[['使用客户数', '使用总量', '产品代码']]
                )
                ...
                fig.update_layout(
                    xaxis_title="物料名称",
                    yaxis_title="平均ROI",
                    margin=dict(l=40, r=40, t=50, b=200),  # 大幅增加底部边距，确保标签完全显示
                    title_font=dict(size=16, family="PingFang SC", color="#333333"),
                    paper_bgcolor='white',
                    plot_bgcolor='white',
                    font=dict(
            ...
            <div class="chart-explanation">
                <div class="chart-explanation-title">图表解读：</div>
                <p>这个柱状图显示了物料的平均ROI(投资回报率)，可通过上方选择器切换显示最高或最低ROI的物料及数量。柱子越高表示该物料带来的回报越高，不同颜色代表不同物料类别。红色虚线是ROI=1的参考线，低于这条线的物料是亏损的。</p>
            </div>
            ''', unsafe_allow_html=True)

            # 物料费比分析 - 高级专业版重做（移除排序方式选择，默认排序）
            ...
            with col1:
                ...
                if len(distributor_cost_ratio) > 0:
                    ...
                    fig.add_trace(go.Bar(
                        ...
                        hovertemplate='<b>%{y}</b><br>' +
                                      '<b>详细信息:</b><br>' +
                                      '物料费比: <b>%{x:.2f}%</b><br>' +
                                      '物料成本: <b>¥%{customdata[0]:,.2f}</b><br>' +
                                      '销售总额: <b>¥%{customdata[1]:,.2f}</b><br>' +
                                      '客户价值分层: <b>%{customdata[2]}</b><br>' +
                                      'ROI: <b>%{customdata[3]:.2f}</b><br>' +
                                      '客户代码: <b>%{customdata[4]}</b><br>' +
                                      '所属区域: <b>%{customdata[5]}</b>',
                        customdata=np.column_stack((
                            ...
                            top_distributors['所属区域'] if '所属区域' in top_distributors.columns else np.array(
                                ['未知'] * len(top_distributors))
                        ))
                    ))
                ...
                <div class="chart-explanation">
                    <div class="chart-explanation-title">图表解读：</div>
                    <p>此图展示了TOP 10经销商的物料费比(物料成本占销售额的百分比)，默认按销售额从高到低排序。
                ...
                if '所属区域' in filtered_distributor.columns:
                    ...
                    if len(region_cost_ratio) > 0:
                        ...
                        fig.add_trace(go.Bar(
                            ...
                            textposition='outside',
                            textfont=dict(size=12, family="PingFang SC", color="#333333"),
                            hovertemplate='<b>%{x}</b><br>' +
                                          '<b>详细信息:</b><br>' +
                                          '平均费比: <b>%{y:.2f}%</b><br>' +
                                          '综合费比: <b>%{customdata[0]:.2f}%</b><br>' +
                                          '物料总成本: <b>¥%{customdata[1]:,.2f}</b><br>' +
                                          '销售总额: <b>¥%{customdata[2]:,.2f}</b><br>' +
                                          '经销商数量: <b>%{customdata[3]}</b>',
                            customdata=region_cost_ratio[['综合费比', '物料总成本', '销售总额', '经销商数量']]
                        ))
                        ...
                        fig.add_trace(go.Scatter(
                            ...
                            yaxis='y2',
                            hovertemplate='<b>%{x}</b><br>经销商数量: <b>%{y}</b><br>'
                        ))
                ...
                fig.add_trace(
                    go.Bar(
                        ...
                        hovertemplate='<b>%{x}</b><br>' +
                                      '<b>详细信息:</b><br>' +
                                      '物料成本: <b>¥%{y:,.2f}</b><br>' +
                                      '销售金额: <b>¥%{customdata[0]:,.2f}</b><br>' +
                                      'ROI: <b>%{customdata[1]:.2f}</b><br>' +
                                      '物料费比: <b>%{customdata[2]:.2f}%</b>',
                        customdata=monthly_data[['销售金额', 'ROI', '物料销售比率']]
                    ),
                    secondary_y=False
                )
                ...
                fig.add_trace(
                    go.Bar(
                        ...
                        hovertemplate='<b>%{x}</b><br>' +
                                      ...
                                      'ROI: <b>%{customdata[1]:.2f}</b><br>' +
                                      '物料费比: <b>%{customdata[2]:.2f}%</b>',
                        customdata=monthly_data[['物料成本', 'ROI', '物料销售比率']]
                    ),
                    secondary_y=False
                )
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        hovertemplate='<b>%{x}</b><br>' +
                                      ...
                                      'ROI: <b>%{y:.2f}</b><br>' +
                                      '销售金额: <b>¥%{customdata[0]:,.2f}</b><br>' +
                                      '物料成本: <b>¥%{customdata[1]:,.2f}</b><br>' +
                                      '物料费比: <b>%{customdata[2]:.2f}%</b>',
                        customdata=monthly_data[['销售金额', '物料成本', '物料销售比率']]
                    ),
                    secondary_y=True
                )
