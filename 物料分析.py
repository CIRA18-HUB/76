...
def create_sidebar_filters(material_data, sales_data, distributor_data):
    """创建更美观直观的侧边栏筛选器并返回筛选后的数据"""

    # 设置侧边栏标题和样式 - 更简洁现代的设计
    st.sidebar.markdown(
        '<div style="text-align: center; padding: 12px 0; margin-bottom: 24px; border-bottom: 1px solid #E0E4EA;">'
        '<h3 style="color: #2B5AED; font-size: 18px; margin: 0; font-weight: 600;">物料投放分析</h3>'
        '<p style="color: #646A73; font-size: 13px; margin: 5px 0 0 0;">智能筛选面板</p>'
        '</div>',
    ...
    with col1:
        st.markdown("<span style='font-weight: 500;'>选择区域</span>", unsafe_allow_html=True)
    with col2:
        all_regions = st.checkbox("全选", value=True, key="all_regions")
    ...
    if selected_regions:
        st.sidebar.markdown(
            f'<div class="sidebar-selection-info">已选择 {len(selected_regions)}/{len(regions)} 个区域</div>',
            unsafe_allow_html=True
        )

        # 显示动态区域徽章
        badges_html = ""
        for region in selected_regions[:3]:  # 最多显示3个徽章
        ...
        # 显示其他筛选器
        st.session_state.filter_state['show_provinces'] = True
        st.session_state.filter_state['show_categories'] = True
        st.session_state.filter_state['show_sales_persons'] = True
    else:
        st.sidebar.markdown(
            '<div class="sidebar-filter-description">请至少选择一个区域以继续筛选</div>',
            unsafe_allow_html=True
        )
        st.session_state.filter_state['show_provinces'] = False
        st.session_state.filter_state['show_categories'] = False
        st.session_state.filter_state['show_sales_persons'] = False

    # 省份筛选 - 仅当选择了区域时显示
    if st.session_state.filter_state['show_provinces']:
        st.sidebar.markdown('<div class="sidebar-filter-heading">省份筛选</div>', unsafe_allow_html=True)

        # 根据所选区域动态过滤可选省份
        provinces_in_selected_regions = material_data[
            material_data['所属区域'].isin(selected_regions)
        ]['省份'].unique()
        provinces = sorted(provinces_in_selected_regions)

        # 全选按钮 - 省份
        col1, col2 = st.sidebar.columns([3, 1])
        with col1:
            st.markdown("<span style='font-weight: 500;'>选择省份</span>", unsafe_allow_html=True)
        with col2:
            all_provinces = st.checkbox("全选", value=True, key="all_provinces")
        ...
        if all_provinces:
            ...
            previous_provinces = [p for p in st.session_state.filter_state['provinces'] if p in provinces]
        ...
        if selected_provinces:
            st.sidebar.markdown(
                f'<div class="sidebar-selection-info">已选择 {len(selected_provinces)}/{len(provinces)} 个省份</div>',
                unsafe_allow_html=True
            )

            # 显示动态区域徽章
            badges_html = ""
            for province in selected_provinces[:3]:  # 最多显示3个徽章
            ...
            # 允许继续显示经销商筛选
            st.session_state.filter_state['show_distributors'] = True
        else:
            st.sidebar.markdown(
                '<div class="sidebar-filter-description">请至少选择一个省份以继续筛选</div>',
                unsafe_allow_html=True
            )
            st.session_state.filter_state['show_distributors'] = False

    # 物料类别筛选 - 仅当选择了区域时显示
    if st.session_state.filter_state['show_categories']:
        st.sidebar.markdown('<div class="sidebar-filter-heading">物料筛选</div>', unsafe_allow_html=True)

        # 全选按钮 - 物料类别
        col1, col2 = st.sidebar.columns([3, 1])
        with col1:
            st.markdown("<span style='font-weight: 500;'>选择物料类别</span>", unsafe_allow_html=True)
        with col2:
            all_categories = st.checkbox("全选", value=True, key="all_categories")
        ...
        # 根据全选状态设置默认值
        if all_categories:
            default_categories = material_categories
        else:
            previous_categories = [c for c in st.session_state.filter_state['categories'] if c in material_categories]
        ...
        if selected_categories:
            st.sidebar.markdown(
                f'<div class="sidebar-selection-info">已选择 {len(selected_categories)}/{len(material_categories)} 个物料类别</div>',
                unsafe_allow_html=True
            )

            # 显示动态类别徽章
            badges_html = ""
            for category in selected_categories[:3]:  # 最多显示3个徽章
    ...
    # 销售人员筛选 - 仅当选择了区域时显示
    if st.session_state.filter_state['show_sales_persons']:
        st.sidebar.markdown('<div class="sidebar-filter-heading">销售团队筛选</div>', unsafe_allow_html=True)

        # 尝试使用"申请人"字段作为销售人员
        if '申请人' in material_data.columns and '销售人员' not in material_data.columns:
            material_data['销售人员'] = material_data['申请人']
            sales_data['销售人员'] = sales_data['申请人'] if '申请人' in sales_data.columns else '未知销售人员'

        # 全选按钮 - 销售人员
        col1, col2 = st.sidebar.columns([3, 1])
        with col1:
            st.markdown("<span style='font-weight: 500;'>选择销售人员</span>", unsafe_allow_html=True)
        with col2:
            all_sales = st.checkbox("全选", value=True, key="all_sales")

        # 获取所有销售人员并根据区域和省份筛选
        filtered_for_sales = material_data[
            material_data['所属区域'].isin(selected_regions)
        ]

        if selected_provinces:
            filtered_for_sales = filtered_for_sales[
                filtered_for_sales['省份'].isin(selected_provinces)
            ]

        sales_persons = sorted(filtered_for_sales['销售人员'].unique())

        # 根据全选状态设置默认值
        if all_sales:
            default_sales = sales_persons
        else:
            previous_sales = [s for s in st.session_state.filter_state['sales_persons'] if s in sales_persons]
            default_sales = previous_sales if previous_sales else []
        ...
        # 更新会话状态
        st.session_state.filter_state['sales_persons'] = selected_sales_persons

        # 显示已选销售人员数量
        if selected_sales_persons:
            st.sidebar.markdown(
                f'<div class="sidebar-selection-info">已选择 {len(selected_sales_persons)}/{len(sales_persons)} 个销售人员</div>',
                unsafe_allow_html=True
            )

            # 显示动态销售人员徽章
            badges_html = ""
            for sales_person in selected_sales_persons[:3]:  # 最多显示3个徽章
                badges_html += f'<span class="sidebar-badge">{sales_person}</span>&nbsp;'
            if len(selected_sales_persons) > 3:
                badges_html += f'<span class="sidebar-badge">+{len(selected_sales_persons) - 3}个</span>'
            st.sidebar.markdown(badges_html, unsafe_allow_html=True)
        else:
            st.sidebar.markdown(
                '<div class="sidebar-filter-description">请至少选择一个销售人员</div>',
                unsafe_allow_html=True
            )

    # 经销商筛选 - 仅当选择了省份时显示
    if st.session_state.filter_state['show_distributors']:
        st.sidebar.markdown('<div class="sidebar-filter-heading">经销商筛选</div>', unsafe_allow_html=True)

        # 根据已选条件筛选经销商
        filtered_for_distributors = distributor_data.copy()

        # 应用销售人员筛选
        if selected_sales_persons:
            filtered_for_distributors = filtered_for_distributors[
                filtered_for_distributors['销售人员'].isin(selected_sales_persons)
            ]

        # 检查是否有"所属区域"和"省份"列
        if '所属区域' in distributor_data.columns and selected_regions:
            filtered_for_distributors = filtered_for_distributors[
                filtered_for_distributors['所属区域'].isin(selected_regions)
            ]

        if '省份' in distributor_data.columns and selected_provinces:
            filtered_for_distributors = filtered_for_distributors[
                filtered_for_distributors['省份'].isin(selected_provinces)
            ]

        distributor_names = sorted(filtered_for_distributors['经销商名称'].unique())

        # 全选按钮 - 经销商
        col1, col2 = st.sidebar.columns([3, 1])
        with col1:
            st.markdown("<span style='font-weight: 500;'>选择经销商</span>", unsafe_allow_html=True)
        with col2:
            all_distributors = st.checkbox("全选", value=True, key="all_distributors")

        # 根据全选状态设置默认值
        if all_distributors:
            default_distributors = distributor_names
        else:
            previous_distributors = [d for d in st.session_state.filter_state['distributors'] if d in distributor_names]
        ...
        if selected_distributors:
            st.sidebar.markdown(
                f'<div class="sidebar-selection-info">已选择 {len(selected_distributors)}/{len(distributor_names)} 个经销商</div>',
                unsafe_allow_html=True
            )
    ...
    # 添加数据下载区域 - 更优雅的设计
    st.sidebar.markdown(
        '<div style="background-color: rgba(43, 90, 237, 0.05); border-radius: 8px; padding: 15px; margin-top: 20px;">'
        '<p style="font-weight: 600; color: #2B5AED; margin-bottom: 10px; font-size: 14px;">数据导出</p>',
        unsafe_allow_html=True
    )
    ...
    # 筛选数据
    if update_button or True:  # 默认自动更新，但保留按钮以便用户手动触发
        # 准备基础筛选条件
        material_filter = (material_data['所属区域'].isin(selected_regions))
        sales_filter = (sales_data['所属区域'].isin(selected_regions))
        distributor_filter = pd.Series(True, index=distributor_data.index)

        if '所属区域' in distributor_data.columns:
            distributor_filter &= distributor_data['所属区域'].isin(selected_regions)

        # 省份筛选（如果已选择）
        if selected_provinces:
            material_filter &= (material_data['省份'].isin(selected_provinces))
            sales_filter &= (sales_data['省份'].isin(selected_provinces))
            if '省份' in distributor_data.columns:
                distributor_filter &= distributor_data['省份'].isin(selected_provinces)

        # 物料类别筛选（如果已选择）
        if selected_categories:
            material_filter &= (material_data['物料类别'].isin(selected_categories))

        # 销售人员筛选（如果已选择）
        if selected_sales_persons:
            material_filter &= (material_data['销售人员'].isin(selected_sales_persons))
            sales_filter &= (sales_data['销售人员'].isin(selected_sales_persons))
            distributor_filter &= (distributor_data['销售人员'].isin(selected_sales_persons))

        # 经销商筛选（如果已选择）
        if selected_distributors:
            distributor_filter &= (distributor_data['经销商名称'].isin(selected_distributors))
            material_filter &= (material_data['经销商名称'].isin(selected_distributors))
            sales_filter &= (sales_data['经销商名称'].isin(selected_distributors))

        # 应用筛选
        filtered_material = material_data[material_filter]
        filtered_sales = sales_data[sales_filter]
        filtered_distributor = distributor_data[distributor_filter]

        # 显示筛选应用状态 - 使用更漂亮的设计
        if update_button:  # 只有手动点击更新时才显示确认
            st.sidebar.markdown(
                f'<div style="background-color: rgba(15, 200, 111, 0.1); border-radius: 4px; padding: 12px; margin-top: 15px;">'
                f'<div style="display: flex; align-items: center; margin-bottom: 8px;">'
                f'<span style="color: #0FC86F; font-weight: 600; font-size: 14px;">✓ 筛选条件已应用</span>'
                f'</div>'
                f'<div style="font-size: 12px; color: #646A73; line-height: 1.6;">'
                f'区域: {len(selected_regions)}/{len(regions)}<br>'
                f'省份: {len(selected_provinces) if selected_provinces else 0}/{len(provinces) if "provinces" in locals() else 0}<br>'
                f'物料类别: {len(selected_categories) if selected_categories else 0}/{len(material_categories) if "material_categories" in locals() else 0}<br>'
                f'销售人员: {len(selected_sales_persons) if selected_sales_persons else 0}/{len(sales_persons) if "sales_persons" in locals() else 0}<br>'
                f'经销商: {len(selected_distributors) if selected_distributors else 0}/{len(distributor_names) if "distributor_names" in locals() else 0}'
                f'</div>'
                f'</div>',
                unsafe_allow_html=True
            )
    ...
    if material_download:
        csv = filtered_material.to_csv(index=False).encode('utf-8-sig')
    ...
    if sales_download:
        csv = filtered_sales.to_csv(index=False).encode('utf-8-sig')
        st.sidebar.download_button(
            label="点击下载销售数据",
            data=csv,
    ...
    return filtered_material, filtered_sales, filtered_distributor
...
def main():
        ...
        with tab2:
            ...
            if len(material_roi) > 0:
                ...
                fig.update_layout(
                    xaxis_title="物料名称",
                    yaxis_title="平均ROI",
                    margin=dict(l=20, r=20, t=40, b=200),  # 大幅增加底部边距，确保标签完全显示
                    paper_bgcolor='white',
                    plot_bgcolor='white',
                    font=dict(
                        family="PingFang SC, Helvetica Neue, Arial, sans-serif",
            ...
            with tab3:
                ...
                with col1:
                    ...
                    def get_optimal_diversity(row):
                        try:
                            ...
                            if isinstance(row, dict) or isinstance(row, pd.Series):
                                if '物料多样性' in row and pd.notna(row['物料多样性']) and '销售规模' in row:
                                    current = row['物料多样性']
                                    if str(row['销售规模']) == '大规模':
                                        optimal = max(8, current * 1.2)
                                    elif str(row['销售规模']) == '中大规模':
                                        optimal = max(6, current * 1.15)
                                    elif str(row['销售规模']) == '中小规模':
                    ...
                    if len(filtered_distributor) > 0:
                        try:
                            # 检查是否有足够的数据进行分组
                            if len(filtered_distributor) >= 4:  # 需要至少4条数据才能有效分成4组
                                # 初始化变量，防止未定义错误
                                scale_metrics = pd.DataFrame()

                                # 获取分位数，但先检查数据是否足够
                                if filtered_distributor['销售总额'].nunique() >= 4:
                                    sales_quartiles = filtered_distributor['销售总额'].quantile(
                                        [0.25, 0.5, 0.75]).tolist()

                                    # 确保分位数不重复且递增
                                    unique_quartiles = sorted(list(set([0] + sales_quartiles + [float('inf')])))

                                    # 确保有足够的独特值创建分组
                                    if len(unique_quartiles) >= 3:  # 至少需要3个独特值才能创建2个分组
                                        # 根据实际独特值数量确定标签
                                        group_labels = ['小规模', '中小规模', '中大规模', '大规模'][
                                                       :len(unique_quartiles) - 1]
                                        ...
                                        # 检查分组是否成功
                                        if '销售规模' in temp_df.columns and not temp_df['销售规模'].isna().all():
                                            # 计算各规模经销商的平均指标
                                            scale_metrics = temp_df.groupby('销售规模', observed=True).agg({
                                                'ROI': 'mean',
                                                '物料多样性': 'mean',
                                                '物料销售比率': 'mean',
                                                '经销商名称': 'count'
                                ...
                                if len(scale_metrics) == 0:
                                    ...
                                    scale_metrics = temp_df.groupby('销售规模', observed=False).agg({
                                        'ROI': 'mean',
                                        '物料多样性': 'mean',
                                        '物料销售比率': 'mean',
                                        '经销商名称': 'count'
                                ...
                                # 如果还是没有数据，显示提示并退出
                                if len(scale_metrics) == 0:
                                    st.info("数据分组后无结果，无法创建图表")
                                else:
                                    scale_metrics = scale_metrics.rename(columns={'经销商名称': '经销商数量'})

                                    # 设置建议的物料多样性
                                    scale_metrics['建议物料多样性'] = scale_metrics.apply(get_optimal_diversity, axis=1)

                                    # 确保需要的列都存在
                                    if all(col in scale_metrics.columns for col in
                                           ['销售规模', '物料多样性', '建议物料多样性', '经销商数量']):
                                        # 创建更直观的图表代替表格
                                        fig = px.bar(
                                            scale_metrics,
                                            x='销售规模',
                                        ...
                                        fig.add_trace(
                                            go.Scatter(
                                                ...
                                                mode='markers+text',
                                                marker=dict(size=scale_metrics['经销商数量'] * 2 + 10,
                                                            color='rgba(255, 170, 0, 0.6)'),
                                        ...
                                        fig.update_layout(
                                            ...
                                            margin=dict(l=50, r=70, t=50, b=100),  # 增加边距，特别是底部
                                            paper_bgcolor='white',
                                            plot_bgcolor='white'
                                        )
                                        ...
                                        st.info("所需列缺失，无法创建图表")
                            else:
                                st.info("数据量不足，需要至少4条数据才能创建分析")
                        except Exception as e:
                            st.error(f"创建图表时出错: {str(e)}")
                            st.info("无法创建销售规模分析图表")
                    else:
                        st.info("暂无经销商数据。")
                    ...
                    if len(filtered_distributor) > 0:
                        try:
                            ...
                            if len(diversity_gap) > 0:
                                ...
                                fig.update_layout(
                                    height=420,  # 增加高度以容纳标签
                                    margin=dict(l=180, r=70, t=50, b=50),  # 大幅增加左侧边距以容纳经销商名称
                                    paper_bgcolor='white',
                                    plot_bgcolor='white',
                                    xaxis=dict(
                                        showgrid=True,
                    ...
                    if len(group_avg) > 0:
                        ...
                        with col1:
                            ...
                            if len(high_eff_avg) > 0:
                                ...
                                fig.update_layout(
                                    height=380,  # 增加高度
                                    margin=dict(l=30, r=30, t=50, b=50),  # 增加所有边距
                                    paper_bgcolor='white',
                                    legend=dict(
                                        orientation="v",
                                        yanchor="middle",
        ...
        with tab4:
            ...
            with col1:
                ...
                # 创建物料组合ROI分析图表
                # 从推荐中提取具体的物料组合而不是单一类别
                combinations_data = []

                if material_recommendations and material_recommendations[0][
                    "推荐名称"] != "暂无足够数据生成物料组合优化建议":
                    for rec in material_recommendations[:5]:  # 使用前5个推荐
                        if "核心类别" in rec and len(rec["核心类别"]) >= 2:
                            combo_name = " + ".join(rec["核心类别"][:2])
                            roi = float(rec["预期ROI"]) if "预期ROI" in rec else 0
                            combinations_data.append({"组合名称": combo_name, "预期ROI": roi, "推荐": "是"})

                # 如果没有足够数据，添加默认组合
                if len(combinations_data) < 2:
                    default_combos = [
                        {"组合名称": "促销物料 + 陈列物料", "预期ROI": 3.15, "推荐": "是"},
                        {"组合名称": "宣传物料 + 赠品", "预期ROI": 2.87, "推荐": "是"},
                        {"组合名称": "陈列物料 + 包装物料", "预期ROI": 2.65, "推荐": "是"}
                    ]
                    combinations_data.extend(default_combos)
                ...
                fig.update_layout(
                    height=400,
                    margin=dict(l=20, r=20, t=40, b=120),  # 增加底部边距
                    paper_bgcolor='white',
                    plot_bgcolor='white',
                    font=dict(
                        family="PingFang SC, Helvetica Neue, Arial, sans-serif",
                ...
                <div class="chart-explanation">
                    <div class="chart-explanation-title">图表解读：</div>
                    <p>此图展示了不同物料组合的预期ROI值。绿色柱子表示推荐使用的物料组合，这些组合具有较高的ROI，能产生更好的投资回报。红色虚线表示ROI=1的盈亏平衡点，组合的ROI值越高，表示每投入1元物料能产生更多的销售额。</p>
                </div>
                ''', unsafe_allow_html=True)
            ...
            with col2:
                ...
                if material_recommendations and material_recommendations[0][
                    ...
                    for i, recommendation in enumerate(material_recommendations[:2]):
                        # 确保核心类别存在且不是空列表
                        core_categories = recommendation.get("核心类别", [])
                        if not core_categories:
                            core_categories = ["促销物料", "陈列物料"]

                        combo_name = ' + '.join(core_categories[:2])
                        expected_roi = recommendation.get("预期ROI", "1.8-2.2")
                        expected_increase = recommendation.get("预计销售提升", "20-25%")
                        best_match = recommendation.get("最佳搭配物料", "主要展示物料 + 辅助促销物料")

                        # 计算进度条宽度
                        try:
                            roi_value = float(expected_roi) if isinstance(expected_roi, (int, float)) or (
                                        isinstance(expected_roi, str) and expected_roi.replace('.', '',
                                                                                               1).isdigit()) else 2.0
                            progress_width = min(roi_value / 3 * 100, 100)
                        except:
                            progress_width = 66  # 默认值
                        ...
                        <div style="background-color: rgba(43, 90, 237, 0.05); border-radius: 8px; padding: 16px; margin-bottom: 16px;">
                            ...
                            <div style="margin-bottom: 8px;">
                                ...
                                <span style="color: #0FC86F; font-weight: 600; font-size: 15px;"> {expected_roi}</span>
                            </div>
                            ...
                            <div style="margin-bottom: 6px; font-size: 13px;">
                                ...
                                <span style="font-weight: 500;"> {expected_increase}</span>
                            </div>
                            <div style="margin-bottom: 6px; font-size: 13px;">
                                <span style="color: #646A73;">推荐配比:</span>
                                <span style="font-weight: 500;"> {best_match}</span>
                            </div>
                        </div>
                        ''', unsafe_allow_html=True)
                else:
                    ...
                    <div style="background-color: rgba(43, 90, 237, 0.05); border-radius: 8px; padding: 16px; margin-bottom: 16px;">
                        ...
                        <div style="margin-bottom: 8px;">
                            <span style="color: #646A73; font-size: 13px;">预期ROI:</span>
                            <span style="color: #0FC86F; font-weight: 600; font-size: 15px;"> 3.1</span>
                        </div>
                        <div class="feishu-progress-container" style="margin-bottom: 12px;">
                            <div class="feishu-progress-bar" style="width: 85%; background-color: #0FC86F;"></div>
                        </div>
                        <div style="margin-bottom: 6px; font-size: 13px;">
                            <span style="color: #646A73;">预计提升:</span>
                    ...
                    <div style="background-color: rgba(43, 90, 237, 0.05); border-radius: 8px; padding: 16px; margin-bottom: 16px;">
                        ...
                        <div style="margin-bottom: 8px;">
                            <span style="color: #646A73; font-size: 13px;">预期ROI:</span>
                            <span style="color: #0FC86F; font-weight: 600; font-size: 15px;"> 2.8</span>
                        </div>
                        <div class="feishu-progress-container" style="margin-bottom: 12px;">
                            <div class="feishu-progress-bar" style="width: 75%; background-color: #0FC86F;"></div>
                        </div>
                        <div style="margin-bottom: 6px; font-size: 13px;">
                            <span style="color: #646A73;">预计提升:</span>
            ...
            with col1:
                ...
                try:
                    # 尝试从数据中提取具体产品和物料
                    product_codes = material_data['产品代码'].unique()[:8]  # 取前8个产品代码
                    material_codes = material_data['产品代码'].unique()[-8:]  # 取后8个物料代码
                    ...
                    fig.update_layout(
                        ...
                        margin=dict(l=20, r=20, t=10, b=100),  # 增加底部边距以容纳较长的标签
                        paper_bgcolor='white',
                        plot_bgcolor='white',
                        coloraxis_colorbar=dict(
                            title="匹配得分",
