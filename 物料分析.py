...
def create_distributor_analysis_tab(filtered_distributor, material_data, sales_data):
    ...
    # 检查是否存在必要的列
    required_cols = ['客户代码', '产品代码', '物料类别'] if len(material_data) > 0 else []
    missing_cols = [col for col in required_cols if col not in material_data.columns]
    if missing_cols:
        st.warning(f"物料数据缺少以下列: {', '.join(missing_cols)}，部分分析功能将受限。")
    ...
    # 计算关键指标
    total_distributors = filtered_distributor['经销商名称'].nunique()
    avg_roi = filtered_distributor['ROI'].mean()
    avg_material_cost = filtered_distributor['物料总成本'].mean()
    avg_sales = filtered_distributor['销售总额'].mean()

    # 客户分层统计
    segment_counts = filtered_distributor['客户价值分层'].value_counts().to_dict()
        ...
        if len(segment_counts) > 0:
            ...
            fig.update_traces(
                ...
                hovertemplate='<b>%{label}</b><br>' +
                              '数量: %{value}家<br>' +
                              '占比: %{customdata[0]:.2f}%<br>' +
                              '平均ROI: %{customdata[1]:.2f}<br>' +
                              '平均销售额: ¥%{customdata[2]:,.2f}'
            )
    ...
    # ===================== 第二部分：经销商价值与投入分析 =====================
    st.markdown('<div class="feishu-chart-title" style="margin-top: 20px;">经销商价值与投入深度分析</div>',
                unsafe_allow_html=True)
    ...
    # 确保关键指标存在
    required_cols = ['ROI', '物料总成本', '销售总额', '物料销售比率', '经销商名称', '客户价值分层', '客户代码']
    ...
    if missing_cols:
        ...
        # 准备数据
        matrix_data = filtered_distributor.copy()

        # 计算物料多样性 - 如果数据中不存在
        if '物料多样性' not in matrix_data.columns and '产品代码' in material_data.columns:
            # 计算每个经销商使用的物料种类数
            material_diversity = material_data.groupby('经销商名称')['产品代码'].nunique().reset_index()
            material_diversity.columns = ['经销商名称', '物料多样性']

            # 合并到主数据
            matrix_data = pd.merge(matrix_data, material_diversity, on='经销商名称', how='left')
            matrix_data['物料多样性'] = matrix_data['物料多样性'].fillna(0)

        # 创建四象限散点图 - x轴销售额，y轴ROI
        fig = px.scatter(
            matrix_data,
            x='销售总额',
            y='ROI',
        ...
        fig.update_traces(
            hovertemplate='<b>%{hovertext}</b><br>' +
                          '<span style="font-weight:600;">财务指标:</span><br>' +
                          '销售总额: ¥%{x:,.2f}<br>' +
                          '物料产出比: %{y:.2f}<br>' +
                          '物料投入: ¥%{marker.size:,.2f}<br>' +
                          '物料费比: %{customdata[0]:.2f}%<br><br>' +
                          '<span style="font-weight:600;">客户信息:</span><br>' +
                          '客户价值分层: %{marker.color}<br>' +
                          '物料多样性: %{customdata[1]:.0f}种<br>' +
                          '客户编码: %{customdata[2]}',
            customdata=np.column_stack((
                matrix_data['物料销售比率'],
                matrix_data['物料多样性'] if '物料多样性' in matrix_data.columns else np.zeros(len(matrix_data)),
                matrix_data['客户代码']
            )),
            marker=dict(
                opacity=0.8,
        ...
        # 添加平均值参考线
        avg_sales = matrix_data['销售总额'].mean()
        avg_roi = matrix_data['ROI'].mean()

        # 添加水平参考线
        fig.add_shape(
            type="line",
            x0=matrix_data['销售总额'].min() * 0.9,
            x1=matrix_data['销售总额'].max() * 1.1,
            y0=avg_roi,
            y1=avg_roi,
            line=dict(color="#2B5AED", width=1.5, dash="dash")
        )
        ...
        fig.add_shape(
            ...
            x0=avg_sales,
            x1=avg_sales,
            y0=matrix_data['ROI'].min() * 0.9,
            y1=matrix_data['ROI'].max() * 1.1,
            line=dict(color="#2B5AED", width=1.5, dash="dash")
        )

        # 添加象限标签
        fig.add_annotation(
            x=matrix_data['销售总额'].max() * 0.75,
            y=matrix_data['ROI'].max() * 0.75,
            text="高价值区<br>(高产出比 + 高销售)",
            showarrow=False,
        ...
        fig.add_annotation(
            x=matrix_data['销售总额'].min() * 1.5,
            y=matrix_data['ROI'].max() * 0.75,
            text="潜力区<br>(高产出比 + 低销售)",
            showarrow=False,
            font=dict(size=12, color="#FFAA00", family="PingFang SC"),
            bgcolor="rgba(255, 255, 255, 0.7)",
            bordercolor="#FFAA00",
            borderwidth=1,
            borderpad=4
        ...
        fig.add_annotation(
            x=matrix_data['销售总额'].max() * 0.75,
            y=matrix_data['ROI'].min() * 1.5,
            text="优化区<br>(低产出比 + 高销售)",
            showarrow=False,
            font=dict(size=12, color="#2B5AED", family="PingFang SC"),
            bgcolor="rgba(255, 255, 255, 0.7)",
            bordercolor="#2B5AED",
            borderwidth=1,
            borderpad=4
        ...
        fig.add_annotation(
            x=avg_sales,
            y=matrix_data['ROI'].min() * 0.95,
            text=f"平均销售额: ¥{avg_sales:,.0f}",
            showarrow=False,
            font=dict(size=10, color="#2B5AED", family="PingFang SC"),
            bgcolor="rgba(255, 255, 255, 0.7)",
            bordercolor="#2B5AED",
            borderwidth=1,
            borderpad=2,
        ...
        fig.add_annotation(
            x=matrix_data['销售总额'].min() * 0.95,
            y=avg_roi,
            text=f"平均产出比: {avg_roi:.2f}",
            showarrow=False,
            font=dict(size=10, color="#2B5AED", family="PingFang SC"),
            bgcolor="rgba(255, 255, 255, 0.7)",
            bordercolor="#2B5AED",
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                ...
                type="log" if matrix_data['销售总额'].max() / matrix_data['销售总额'].min() > 100 else "linear"
            ),
            yaxis=dict(
                showgrid=True,
                gridcolor='#E0E4EA',
        ...
        <div class="chart-explanation">
            ...
            <p>此矩阵图展示了经销商的价值分布，横轴是销售额，纵轴是物料产出比，气泡大小代表物料投入规模。
            通过蓝色虚线（平均值）划分四个象限：高价值区（右上）是最理想的客户；潜力区（左上）是小而精的客户，
            有扩大规模的潜力；优化区（右下）需要提高物料使用效率；低效区（左下）需要全面改进或考虑调整合作方式。
            悬停可查看详细数据，包括经销商名称、编码以及各项关键指标。</p>
        </div>
        ''', unsafe_allow_html=True)
                ...
                if '物料类别' in top_distributor_materials.columns:
                    for col in material_usage_pct.columns:
                        if col != '客户代码' and col in row:
                            material_data.append(f"{col}: {row[col]:.1f}%")

                # 合并所有数据
                all_data = base_data + [", ".join(material_data[:3])]  # 只展示前3种主要物料
    ...
    with col2:
        ...
        if missing_cols:
            ...
            if len(inefficient_distributors) > 0:
                # 获取这些经销商的物料使用情况 - 从原始数据中提取
                bottom_distributor_codes = inefficient_distributors[
                    '客户代码'].unique() if '客户代码' in inefficient_distributors.columns else []
                ...
                if len(bottom_distributor_codes) > 0 and len(material_data) > 0 and '客户代码' in material_data.columns:
                    ...
                    bottom_distributor_materials = pd.DataFrame()  # 创建空DataFrame
                    if len(bottom_distributor_codes) > 0:
                        st.info("无法获取低效经销商的物料使用数据。")

                # 继续进行后续分析，但确保有数据安全检查
                has_material_category = len(
                    bottom_distributor_materials) > 0 and '物料类别' in bottom_distributor_materials.columns
                ...
                for _, row in inefficient_distributors.iterrows():
                    ...
                    # 物料使用分布数据
                    material_data = []
                    if '物料类别' in bottom_distributor_materials.columns:
                        for col in material_usage_pct.columns:
                            if col != '客户代码' and col in row:
                                material_data.append(f"{col}: {row[col]:.1f}%")
    ...
    with col2:
        ...
        # 分析不同区域的物料使用偏好
        if '所属区域' in filtered_distributor.columns and '所属区域' in material_data.columns and '物料类别' in material_data.columns:
            # 计算各区域各物料类别的使用情况
            ...
            region_diversity = material_data.groupby(['所属区域', '经销商名称', '产品代码']).size().reset_index(
                name='counts')
            region_diversity = region_diversity.groupby(['所属区域', '经销商名称'])['产品代码'].nunique().reset_index()
            ...
            st.warning("数据中缺少区域或物料类别信息，无法生成区域物料偏好分析。")
    ...
    if '所属区域' in filtered_distributor.columns and '客户价值分层' in filtered_distributor.columns:
            ...
            fig = go.Figure(data=[go.Table(
                ...
                cells=dict(
                    values=[
                        ...
                        final_region_data['高价值客户'].apply(lambda x: f"{x:.1f}%"),
                        final_region_data['成长型客户'].apply(lambda x: f"{x:.1f}%"),
                        final_region_data['稳定型客户'].apply(lambda x: f"{x:.1f}%"),
                        final_region_data['低效型客户'].apply(lambda x: f"{x:.1f}%"),
                    ...
                    fill_color=[
                        ...
                        [  # 高价值客户占比 - 颜色渐变
                            'rgba(15, 200, 111, ' + str(min(x / 50, 1)) + ')'
                            for x in final_region_data['高价值客户']
                        ],
                        [  # 成长型客户占比 - 颜色渐变
                            'rgba(43, 90, 237, ' + str(min(x / 50, 1)) + ')'
                            for x in final_region_data['成长型客户']
                        ],
                        [  # 稳定型客户占比 - 颜色渐变
                            'rgba(255, 170, 0, ' + str(min(x / 50, 1)) + ')'
                            for x in final_region_data['稳定型客户']
                        ],
                        [  # 低效型客户占比 - 颜色渐变
                            'rgba(245, 63, 63, ' + str(min(x / 50, 1)) + ')'
                            for x in final_region_data['低效型客户']
                        ],
                        [  # 优化建议 - 按类别着色
        ...
        with col2:
            ...
            selected_region = st.selectbox(
                "选择区域查看详细建议:",
                options=final_region_data['所属区域'].tolist()
            )
            ...
            st.markdown(f'''
            <div style="background-color: rgba({','.join(str(int(int('0x' + bg_color[i:i + 2], 16))) for i in (1, 3, 5))}, 0.1); 
                ...
                <div style="margin-top: 12px; font-size: 13px; color: #646A73;">
                    ...
                    <b>客户结构:</b><br>
                    • 高价值客户: <b style="color: #0FC86F">{selected_data['高价值客户']:.1f}%</b><br>
                    • 成长型客户: <b style="color: #2B5AED">{selected_data['成长型客户']:.1f}%</b><br>
                    • 稳定型客户: <b style="color: #FFAA00">{selected_data['稳定型客户']:.1f}%</b><br>
                    • 低效型客户: <b style="color: #F53F3F">{selected_data['低效型客户']:.1f}%</b>
                </div>
            </div>
            ''', unsafe_allow_html=True)
...
def create_expense_ratio_analysis(filtered_distributor):
    """Creates an improved version of the material expense ratio analysis with no overlapping issues"""
    ...
    with col1:
        ...
        # Calculate material expense ratio for each distributor
        ...
        if distributor_cost_ratio is not None and len(distributor_cost_ratio) > 0:
            ...
            if len(distributor_cost_ratio) > 0:
                # Sort by sales - select top 15 rather than 10 to show more data
                top_distributors = distributor_cost_ratio.sort_values('销售总额', ascending=False).head(15)

                # Ensure ratio keeps two decimal places
                top_distributors['物料销售比率'] = top_distributors['物料销售比率'].round(2)

                # Create a unique marker color for each distributor based on customer value
                color_map = {
                    '高价值客户': '#34C759',
                    '成长型客户': '#007AFF',
                    '稳定型客户': '#FF9500',
                ...
                top_distributors['颜色'] = top_distributors['客户价值分层'].map(lambda x: color_map.get(x, '#777777'))

                # Create expense ratio bar chart - Advanced design
                fig = go.Figure()
                ...
                fig.update_layout(
                    ...
                    height=500,  # Increase height to accommodate 15 records
                    xaxis=dict(
                        title=dict(
                            text="物料费比 (%)",
                            font=dict(
                    ...
                    yaxis=dict(
                        ...
                        ticktext=[f"{name[:12]}..." if len(name) > 15 else name for name in
                                  top_distributors['经销商名称']],
                        tickfont=dict(
                            size=11,
                            family="PingFang SC",
                            color="#333333"
                    ...
                    margin=dict(
                        l=120,  # Reduce left margin to avoid overlapping
                        r=60,
                        t=50,
                        b=60
                    ),
                ...
                # Add customer value layer markers - Improved position
                for i, row in top_distributors.iterrows():
                    # Use background color blocks instead of overlapping text
                    if i < len(top_distributors):
                        fig.add_shape(
                            type="rect",
                            x0=0,
                            y0=i - 0.4,
                            x1=2,  # Only add small color block at starting position
                            y1=i + 0.4,
                            fillcolor=row['颜色'],
                            opacity=0.3,
                            line=dict(width=0),
    ...
    with col2:
        ...
        # Show top and bottom distributors by expense ratio
        if distributor_cost_ratio is not None and len(distributor_cost_ratio) > 0:
            # Filter valid data and sort by expense ratio
            valid_data = distributor_cost_ratio[
                (distributor_cost_ratio['物料销售比率'] > 0) &
                (distributor_cost_ratio['物料销售比率'] < 100)  # Filter extreme values
                ]

            if len(valid_data) > 0:
                # For aesthetics, show at most 30 distributors
                max_display = 30
                if len(valid_data) > max_display:
                    # Select the lowest and highest expense ratio distributors
                    ...
                    valid_data = pd.concat([low_ratio, high_ratio])
                    valid_data = valid_data.sort_values('物料销售比率')  # Sort for display
                else:
                    valid_data = valid_data.sort_values('物料销售比率')

                # Define the color map for customer value segments
                color_map = {
                    '高价值客户': '#34C759',  # green
                    '成长型客户': '#007AFF',  # blue
                    '稳定型客户': '#FF9500',  # orange
                ...
                # Prepare data for table-style display
                distributor_names = valid_data['经销商名称'].tolist()
                ratio_values = valid_data['物料销售比率'].values
                customer_segments = valid_data['客户价值分层'].tolist()
                ...
                fig.add_trace(go.Table(
                    header=dict(
                        values=['经销商名称', '物料费比', '客户类型'],
                        fill_color='rgba(224, 228, 234, 0.6)',
                        align='center',
                        font=dict(size=14, color="#333333", family="PingFang SC"),
                        height=30
                    ),
                    cells=dict(
                        values=[
                        ...
                        fill_color=[
                            ...
                            [
                                'rgba(52, 199, 89, 0.2)' if r <= 30 else
                                'rgba(255, 204, 0, 0.2)' if r <= 40 else
                                'rgba(255, 149, 0, 0.2)' if r <= 50 else
                                'rgba(255, 59, 48, 0.2)' for r in ratio_values
                            ],
                            # Color based on segment
                            [
                                'rgba(52, 199, 89, 0.2)' if s == '高价值客户' else
                                'rgba(0, 122, 255, 0.2)' if s == '成长型客户' else
                                'rgba(255, 149, 0, 0.2)' if s == '稳定型客户' else
                                'rgba(255, 59, 48, 0.2)' for s in customer_segments
                            ]
                        ],
                        align=['left', 'center', 'center'],
                        font=dict(size=12, color="#333333", family="PingFang SC"),
                        height=25
                    )
                ))
        ...
        <div class="chart-explanation">
            <div class="chart-explanation-title">图表解读：</div>
            <p>这个表格展示了物料费比最低和最高的经销商对比。费比越低(绿色背景)表示物料利用效率越高；
            费比越高(红色背景)表示物料使用效率较低。右侧的客户类型列展示了不同价值层级的客户，绿色为高价值客户，蓝色为成长型客户，
            橙色为稳定型客户，红色为低效型客户。通过对比可找出最佳实践和需要改进的案例。</p>
        </div>
        ''', unsafe_allow_html=True)
