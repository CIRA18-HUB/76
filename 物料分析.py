        ...
        with tab3:
                ...
                with col1:
                    ...
                    def get_optimal_diversity(row):
                        try:
                            # 确保物料多样性存在且为有效数值
                            if isinstance(row, dict) and '物料多样性' in row and pd.notna(
                                    row['物料多样性']) and '销售规模' in row:
                                current = row['物料多样性']
                                if row['销售规模'] == '大规模':
                                ...
                                try:
                                    ...
                                    if filtered_distributor['销售总额'].nunique() >= 4:
                                        ...
                                        if len(unique_quartiles) >= 3:  # 至少需要3个独特值才能创建2个分组
                                            ...
                                            # 检查分组是否成功
                                            if '销售规模' in filtered_distributor.columns and not filtered_distributor[
                                                '销售规模'].isna().all():
                                                # 计算各规模经销商的平均指标
                                                ...
                                                if len(scale_metrics) == 0:
                                                    st.info("分组后没有足够的数据进行分析")
                                                else:
                                                    scale_metrics.rename(columns={'经销商名称': '经销商数量'},
                                                                         inplace=True)

                                                    # 设置建议的物料多样性
                                                    scale_metrics['建议物料多样性'] = scale_metrics.apply(
                                                        get_optimal_diversity, axis=1)
                                                    ...
                                                    if all(col in scale_metrics.columns for col in
                                                        ...
                                                        st.info("无法创建销售规模分析图表：缺少必要的列")
                                            else:
                                                st.info("分组过程产生了无效结果，可能是因为销售额分布不足")
                                        else:
                                            st.info("销售额分布不足以创建有效分组")
                                    else:
                                        st.info("销售额数值过于集中，无法进行有效的四分位分组")
                                except Exception as e:
                                    st.warning(f"分组处理错误: {str(e)}")
                                    st.info("使用默认分组展示数据")

                                    # 创建简单分组作为备选
                                    try:
                                        # 简单的二分组
                                        median = filtered_distributor['销售总额'].median()
                                        filtered_distributor['销售规模'] = np.where(
                                            filtered_distributor['销售总额'] > median, '大规模', '小规模')
                                        ...
                                        scale_metrics['建议物料多样性'] = scale_metrics.apply(get_optimal_diversity,
                                                                                              axis=1)
                                        ...
                                        st.error(f"备选分组也失败: {str(e)}")
                                        st.info("无法创建销售规模分析图表: 数据不足")
                            else:
                                st.info("数据量不足，需要至少4条记录才能创建销售规模分组")
                        else:
                            st.info("暂无经销商数据。")
                    except Exception as e:
                        st.error(f"销售规模分析出错: {str(e)}")
