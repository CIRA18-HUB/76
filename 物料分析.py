...
def add_custom_css():
    ...
    <style>
        ...
        .plotly .hover-info {
            ...
            font-family: 'Source Sans Pro', 'PingFang SC', 'Microsoft YaHei', sans-serif !important;
        }
...
def configure_plotly_chart(fig, title, height=600, showlegend=True, legend_orientation="h"):
    ...
    - height: 图表高度(像素)
    - showlegend: 是否显示图例
    - legend_orientation: 图例方向，"h"水平或"v"垂直
...
def create_kpi_card_html(title, value, description, color, percentage=None, prefix="￥", is_percentage=False):
    ...
    <div class="metric-card">
        ...
        <p class="card-value">{display_prefix}{formatted_value}</p>
        <div class="progress-bar">
            <div class="progress-value" style="width: {progress_percentage}%; background-color: {color};"></div>
        </div>
        <p style="font-size: 0.9rem; color: #6c757d; margin-top: 10px;">{description}</p>
    </div>
    """
...
def create_chart_container(chart_figure, title, description, tips, insight_text=None):
    ...
    参数:
    - chart_figure: Plotly图表对象
    - title: 图表标题
    - description: 图表描述
    - tips: 图表使用提示列表
    - insight_text: 数据洞察文本(可选)
    """
    # 创建两列布局：图表和解释
    ...
    with col1:
        # 显示图表
        st.plotly_chart(chart_figure, use_container_width=True, config={"displayModeBar": True, "responsive": True})

    with col2:
        # 显示解释和提示
        st.markdown(f"""
        <div style="background-color: #f8f9fe; border-left: 4px solid #5e72e4; padding: 15px; border-radius: 8px; height: 100%; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
            <h5 style="color: #1f3867; font-size: 1rem; margin-bottom: 10px; border-bottom: 1px solid #eee; padding-bottom: 5px;">
            ...
            <p style="font-size: 0.9rem; margin-bottom: 12px; color: #4a5568; line-height: 1.5;">{description}</p>

            <h5 style="color: #1f3867; font-size: 1rem; margin-bottom: 10px; margin-top: 15px; border-bottom: 1px solid #eee; padding-bottom: 5px;">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 5px;">
            ...
            <ul style="padding-left: 15px; margin-bottom: 12px;">
        """, unsafe_allow_html=True)

        for tip in tips:
            st.markdown(f"""
            <li style="font-size: 0.85rem; margin-bottom: 6px; color: #4a5568; line-height: 1.4;">{tip}</li>
            """, unsafe_allow_html=True)
        ...
        if insight_text:
            st.markdown(f"""
            <div style="background-color: rgba(45, 206, 137, 0.1); border-left: 4px solid #2dce89; padding: 10px; border-radius: 6px; margin-top: 15px;">
                <h6 style="margin: 0 0 8px 0; font-size: 0.95rem; color: #276749; display: flex; align-items: center;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 5px;">
                        <circle cx="12" cy="12" r="10"></circle>
                ...
                <p style="margin: 0; font-size: 0.85rem; color: #2d3748; line-height: 1.5;">{insight_text}</p>
            </div>
            """, unsafe_allow_html=True)
...
def display_fee_ratio_anomalies(anomalies, overall_cost_sales_ratio):
    ...
    if len(anomalies) > 0:
        ...
        <div style='border-radius: 10px; border: 1px solid #63B3ED; padding: 20px; margin-top: 25px; background-color: rgba(66, 153, 225, 0.05);'>
            ...
            <p>共发现<strong>{len(anomalies)}个</strong>费比异常经销商。平均费比为<strong>{overall_cost_sales_ratio:.2f}%</strong>，但这些经销商的费比远高于平均值。</p>
        ...
        <div style='border-radius: 10px; border: 1px solid #c3e6cb; padding: 20px; margin-bottom: 20px; background-color: rgba(72, 187, 120, 0.05);'>
            ...
            <p>当前平均费比为 <strong>{overall_cost_sales_ratio:.2f}%</strong>，继续保持这一水平将有利于提高整体投资回报率。</p>
            ...
            <div style='display: flex; align-items: center; margin: 20px 0;'>
                ...
                <div style='flex-grow: 1; margin-left: 20px;'>
                    <div style='height: 10px; background-color: #EDF2F7; border-radius: 5px; position: relative;'>
                        <div style='position: absolute; height: 100%; width: {min(int(overall_roi * 10), 100)}%; background-color: {
        "#48BB78" if overall_roi >= 5 else "#ECC94B" if overall_roi >= 3 else "#F56565"
