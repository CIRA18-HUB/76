    ...
    if sample_data:
        ...
        try:
            ...
            try:
                ...
                st.success("✓ 成功加载 2025物料源数据.xlsx")
            ...
            try:
                ...
                st.success("✓ 成功加载 25物料源销售数据.xlsx")
            ...
            if '物料类别' in material_price.columns:
                # 确认是否有重复列
                duplicate_cols = material_price.columns.duplicated()
                if any(duplicate_cols):
                    # 重命名重复列
                    material_price.columns = pd.io.parsers.ParserBase(
                        {'names': material_price.columns})._maybe_dedup_names(material_price.columns)

                # 如果物料类别.1存在，将其重命名为物料类别
                if '物料类别.1' in material_price.columns:
                    material_price = material_price.rename(columns={'物料类别.1': '物料类别_分类'})

            # 处理数据
            st.success("✅ 所有数据文件加载成功！正在处理数据...")
            ...
            # 询问用户是否要使用示例数据，而不是自动回退
            st.warning("⚠️ 无法加载真实数据。请确认以下几点：")
            st.markdown("""
            1. 确保三个文件 (2025物料源数据.xlsx, 25物料源销售数据.xlsx, 物料单价.xlsx) 与应用程序在同一目录
            2. 检查文件名是否完全一致（包括大小写）
            3. 如果是在Streamlit Cloud上运行，确保文件已上传到GitHub仓库
            """)
...
def create_sidebar_filters(material_data, sales_data, distributor_data):
        ...
        # 设置侧边栏标题和样式
        st.sidebar.markdown(
            '<div style="text-align: center; padding: 10px 0; margin-bottom: 20px; border-bottom: 1px solid #E0E4EA;">'
            '<h3 style="color: #2B5AED; font-size: 18px; margin: 0;">物料投放分析仪表盘</h3>'
            '<p style="color: #646A73; font-size: 13px; margin: 5px 0 0 0;">数据筛选控制面板</p>'
            '</div>',
            unsafe_allow_html=True
        )

        # 日期筛选部分
        st.sidebar.markdown('<div class="feishu-sidebar-title">时间筛选</div>', unsafe_allow_html=True)

        # 获取所有可用月份并排序
        months = sorted(material_data['月份名'].unique())

        # 默认选择最新月份，但允许用户改变
        default_month_index = len(months) - 1  # 最新月份的索引
        ...
        # 显示已选月份的格式化信息
        st.sidebar.markdown(
            f'<div style="background-color: rgba(43, 90, 237, 0.1); border-radius: 4px; padding: 8px; margin-top: 5px;">'
            f'<span style="color: #2B5AED; font-weight: 500;">当前分析月份:</span> {selected_month}'
            f'</div>',
            unsafe_allow_html=True
        )

        # 区域和省份筛选
        st.sidebar.markdown('<div class="feishu-sidebar-title">区域筛选</div>', unsafe_allow_html=True)

        # 全选按钮 - 区域
        cols = st.sidebar.columns([3, 1])
        with cols[0]:
            st.markdown("**选择区域:**", unsafe_allow_html=True)
        with cols[1]:
        ...
        if selected_regions:
            st.sidebar.markdown(
                f'<div style="font-size: 12px; color: #646A73; margin-top: -10px;">'
                f'已选择 {len(selected_regions)}/{len(regions)} 个区域'
                f'</div>',
                unsafe_allow_html=True
            )
        ...
        cols = st.sidebar.columns([3, 1])
        with cols[0]:
            st.markdown("**选择省份:**", unsafe_allow_html=True)
        with cols[1]:
        ...
        if selected_provinces:
            st.sidebar.markdown(
                f'<div style="font-size: 12px; color: #646A73; margin-top: -10px;">'
                f'已选择 {len(selected_provinces)}/{len(provinces)} 个省份'
                f'</div>',
                unsafe_allow_html=True
            )

        # 物料类别筛选
        st.sidebar.markdown('<div class="feishu-sidebar-title">物料筛选</div>', unsafe_allow_html=True)

        # 全选按钮 - 物料类别
        cols = st.sidebar.columns([3, 1])
        with cols[0]:
            st.markdown("**选择物料类别:**", unsafe_allow_html=True)
        with cols[1]:
        ...
        if selected_categories:
            st.sidebar.markdown(
                f'<div style="font-size: 12px; color: #646A73; margin-top: -10px;">'
                f'已选择 {len(selected_categories)}/{len(material_categories)} 个物料类别'
                f'</div>',
                unsafe_allow_html=True
            )
        ...
        cols = st.sidebar.columns([3, 1])
        with cols[0]:
            st.markdown("**选择销售人员:**", unsafe_allow_html=True)
        with cols[1]:
        ...
        if selected_sales_persons:
            st.sidebar.markdown(
                f'<div style="font-size: 12px; color: #646A73; margin-top: -10px;">'
                f'已选择 {len(selected_sales_persons)}/{len(sales_persons)} 个销售人员'
                f'</div>',
                unsafe_allow_html=True
            )

        # 经销商筛选 - 根据前面已选条件筛选可选经销商
        st.sidebar.markdown('<div class="feishu-sidebar-title">经销商筛选</div>', unsafe_allow_html=True)

        # 根据已选条件筛选经销商
        filtered_for_distributors = distributor_data[
            (distributor_data['月份名'] == selected_month) &
            (distributor_data['销售人员'].isin(selected_sales_persons))
            ]

        # 检查是否有"所属区域"和"省份"列
        if '所属区域' in distributor_data.columns and '省份' in distributor_data.columns:
        ...
        cols = st.sidebar.columns([3, 1])
        with cols[0]:
            st.markdown("**选择经销商:**", unsafe_allow_html=True)
        with cols[1]:
        ...
        if selected_distributors:
            st.sidebar.markdown(
                f'<div style="font-size: 12px; color: #646A73; margin-top: -10px;">'
                f'已选择 {len(selected_distributors)}/{len(distributor_names)} 个经销商'
                f'</div>',
                unsafe_allow_html=True
            )
        ...
        with cols[0]:
            material_download = st.button("下载物料数据", key="dl_material")
        with cols[1]:
            sales_download = st.button("下载销售数据", key="dl_sales")
        ...
        if material_download:
            ...
            st.sidebar.download_button(
                label="点击下载物料数据",
                data=csv,
                file_name=f"物料数据_{selected_month}.csv",
                mime="text/csv",
            )
        ...
        if sales_download:
            ...
            st.sidebar.download_button(
                label="点击下载销售数据",
                data=csv,
                file_name=f"销售数据_{selected_month}.csv",
                mime="text/csv",
            )
        ...
        if update_button or True:  # 默认自动更新
            ...
            # 显示筛选状态提示
            st.sidebar.markdown(
                f'<div style="background-color: rgba(15, 200, 111, 0.1); border-radius: 4px; padding: 8px; margin-top: 20px;">'
                f'<span style="color: #0FC86F; font-weight: 500;">数据筛选已应用</span><br>'
                f'<span style="font-size: 12px;">月份: {selected_month}<br>'
                f'区域: {len(selected_regions)} | 省份: {len(selected_provinces)}<br>'
                f'物料类别: {len(selected_categories)} | 销售人员: {len(selected_sales_persons)}<br>'
                f'经销商: {len(selected_distributors)}</span>'
                f'</div>',
                unsafe_allow_html=True
            )
