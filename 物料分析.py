...
def process_data(material_data, sales_data, material_price):
    ...
    try:
        ...
        if not available_cols:  # a确保至少有一个列可用于分组
    ...
    # 确保所有可能存在NaN的列都有默认值
    distributor_data['物料总成本'] = distributor_data['物料总成本'].fillna(0)
    distributor_data['销售总额'] = distributor_data['销售总额'].fillna(0)
    distributor_data['ROI'] = distributor_data['ROI'].fillna(0)
...
def create_distributor_analysis_tab(filtered_distributor, material_data, sales_data):
    ...
    修复 update_traces 错误、ROI计算逻辑、以及各种显示问题
    """
    # 添加必要的导入
    import pandas as pd
    import numpy as np
    ...
    # 修复图1：限制ROI和物料费比的极端值
    if 'ROI' in filtered_distributor.columns:
        # 限制ROI不超过5（更合理的上限）
        filtered_distributor['ROI'] = filtered_distributor['ROI'].clip(upper=5.0)
    ...
    with col1:
        ...
        if len(segment_counts) > 0:
            ...
            # 确保所有列存在，如果不存在则使用默认值
            has_roi = 'ROI' in segment_counts.columns
            has_sales = '销售总额' in segment_counts.columns

            # 为每一行创建自定义数据
            custom_data = []
            for i, row in segment_counts.iterrows():
                row_data = [
                    row['ROI'] if has_roi else 0,
                    row['销售总额'] if has_sales else 0,
                    segment_distributors[i] if i < len(segment_distributors) else ""
                ]
    ...
    with col2:
        ...
        if '所属区域' in filtered_distributor.columns:
            ...
            # 修复图2：计算物料产出效率，防止除以零的情况
            region_dist['物料产出效率'] = np.where(
                region_dist['总物料成本'] > 0,
                (region_dist['总销售额'] / region_dist['总物料成本']).round(2),
                0
            )
            # 限制上限为5，更合理
            region_dist['物料产出效率'] = region_dist['物料产出效率'].clip(upper=5.0)

            # 计算物料费比
            region_dist['物料费比'] = np.where(
                region_dist['总销售额'] > 0,
                (region_dist['总物料成本'] / region_dist['总销售额'] * 100).round(2),
                0
            )
    ...
    with col1:
        ...
        if missing_cols:
            ...
            # 检查所需列是否存在
            material_usage_pct = pd.DataFrame()
            ...
            # 确认列的存在性
            has_sales_ratio = '物料销售比率' in efficient_distributors.columns
            has_diversity = '物料多样性' in efficient_distributors.columns
            has_region = '所属区域' in efficient_distributors.columns
            has_province = '省份' in efficient_distributors.columns
            has_value_segment = '客户价值分层' in efficient_distributors.columns
            ...
            for _, row in efficient_distributors.iterrows():
                ...
                base_data = [
                    row['销售总额'],
                    row['物料总成本'],
                    row['物料销售比率'] if has_sales_ratio else 0,
                    row['客户价值分层'] if has_value_segment else '未知',
                    row['物料多样性'] if has_diversity else 0,
                    row['所属区域'] if has_region else '未知',
                    row['省份'] if has_province else '未知',
                    row['客户代码']
                ]

                # 物料使用分布数据
                material_data_list = []
                if len(material_usage_pct) > 0:
                    for col in material_usage_pct.columns:
                        if col != '客户代码' and col in row.index:
                            try:
    ...
    with col2:
        ...
        if missing_cols:
            ...
            if len(inefficient_distributors) > 0:
                ...
                # 初始化material_usage_pct DataFrame
                material_usage_pct = pd.DataFrame()

                # 统计各经销商物料类别使用分布
                if '物料类别' in bottom_distributor_materials.columns and len(bottom_distributor_materials) > 0:
                    try:
                        material_usage_by_distributor = bottom_distributor_materials.groupby(
                            ['客户代码', '物料类别']).agg({
                            '求和项:数量（箱）': 'sum'
                        }).reset_index()
                        ...
                        st.warning(f"处理物料类别数据时发生错误: {e}")

                fig = go.Figure()

                # 确认列的存在性
                has_sales_ratio = '物料销售比率' in inefficient_distributors.columns
                has_diversity = '物料多样性' in inefficient_distributors.columns
                has_region = '所属区域' in inefficient_distributors.columns
                has_province = '省份' in inefficient_distributors.columns
                has_value_segment = '客户价值分层' in inefficient_distributors.columns
                ...
                if len(hover_data) > 0:
                    # 适应不同长度行的情况
                    max_len = max(len(item) for item in hover_data)
                    for i in range(len(hover_data)):
                        if len(hover_data[i]) < max_len:
                            hover_data[i] = hover_data[i] + [""] * (max_len - len(hover_data[i]))

                    hover_data = np.array(hover_data)
                else:
                    # 创建一个空的二维数组，确保与hovertemplate兼容
                    hover_data = np.array([[0, 0, 0, "", 0, "", "", "", ""]] * len(inefficient_distributors))

                # 添加条形图 - 修改为直接在添加时设置悬停模板
                fig.add_trace(go.Bar(
    ...
    if '物料多样性' in filtered_distributor.columns and 'ROI' in filtered_distributor.columns:
        ...
        # 修复图5：限制ROI超过5的极端值
        diversity_data['ROI'] = diversity_data['ROI'].clip(upper=5.0)
        ...
        if len(diversity_roi) > 0:
            ...
            # 修复图4：将物料使用偏好分析从物料类别改为物料具体名称
            if '所属区域' in filtered_distributor.columns and len(
                    material_data) > 0 and '产品名称' in material_data.columns:
                try:
                    # 计算各区域各物料名称的使用情况（从物料类别改为产品名称）
                    region_material_usage = material_data.groupby(['所属区域', '产品名称']).agg({
                        '求和项:数量（箱）': 'sum'
                    }).reset_index()
                    ...
                    # 计算占比
                    region_material_usage['使用占比'] = region_material_usage['求和项:数量（箱）'] / \
                                                        region_material_usage['总使用量'] * 100

                    # 为了图表清晰，只保留使用占比较高的物料名称
                    # 对每个区域，按使用量排序取前5个物料名称
                    top_materials_by_region = []
                    for region in region_material_usage['所属区域'].unique():
                        region_data = region_material_usage[region_material_usage['所属区域'] == region]
                        top_n = region_data.sort_values('使用占比', ascending=False).head(5)
                        top_materials_by_region.append(top_n)

                    # 合并所有区域的前5个物料
                    top_region_material_usage = pd.concat(top_materials_by_region)

                    # 创建热力图数据
                    heatmap_data = top_region_material_usage.pivot_table(
                        index='所属区域',
                        columns='产品名称',
                        values='使用占比',
                        aggfunc='sum',
                    ...
                    for i in range(len(fig2.data)):
                        fig2.data[i].customdata = custom_data
                        fig2.data[i].hovertemplate = '<b>%{y} - %{x}</b><br>' + \
                                                     '使用占比: <b>%{z:.2f}%</b><br>' + \
                    ...
                    # 优化热力图间距和网格线
                    for i in range(len(fig2.data)):
                        fig2.data[i].xgap = 4  # 水平间距
                        fig2.data[i].ygap = 4  # 垂直间距
                    ...
                    for i in range(len(heatmap_long['所属区域'].unique()) + 1):
                        fig2.add_shape(
                            type="line",
                            x0=-0.5,
                            x1=len(heatmap_long['产品名称'].unique()) - 0.5,
                            y0=i - 0.5,
                            y1=i - 0.5,
                            line=dict(color="#E0E4EA", width=1)
                        )

                    for i in range(len(heatmap_long['产品名称'].unique()) + 1):
                        fig2.add_shape(
                            type="line",
                            x0=i - 0.5,
                            x1=i - 0.5,
                    ...
                    # 如果热力图创建失败，只显示物料多样性图表
                    st.plotly_chart(fig, use_container_width=True)
                    st.warning(f"创建区域物料使用偏好分析图时发生错误: {e}")
    ...
    if '所属区域' in filtered_distributor.columns and '客户价值分层' in filtered_distributor.columns:
        ...
        # 修复物料产出效率计算，防止除以零
        region_metrics['物料产出效率'] = np.where(
            region_metrics['物料总成本'] > 0,
            (region_metrics['销售总额'] / region_metrics['物料总成本']).round(2),
            0
        )
        # 限制上限为5，更合理
        region_metrics['物料产出效率'] = region_metrics['物料产出效率'].clip(upper=5.0)
        ...
        with col1:
            # 确保有数据可用
            if len(final_region_data) == 0:
                st.info("暂无区域数据可用于分析。")
            else:
                # 创建带热力图效果的表格 - 使用go.Table而不是px.Table
                fig = go.Figure(data=[go.Table(
                    header=dict(
                        values=['<b>区域</b>', '<b>物料<br>产出效率</b>', '<b>平均<br>产出比</b>',
                                '<b>物料<br>费比</b>',
    ...
    # 关闭chart-container div，这个应该在if-else外部
    st.markdown('</div>', unsafe_allow_html=True)
...
def create_material_roi_analysis(filtered_material, filtered_sales):
    """创建单个物料投入产出分析图表 - 修复版：改进计算逻辑并添加明确注释"""
    ...
    # 检查是否同时有物料和销售数据
    has_material_data = filtered_material is not None and len(filtered_material) > 0
    has_sales_data = filtered_sales is not None and len(filtered_sales) > 0
    ...
    if not has_sales_data:
        st.warning("注意：缺少销售数据，将使用物料使用频率进行分析而非产出比。")

        # 当只有物料数据时，分析物料使用情况
        material_usage = filtered_material.groupby(['产品代码', '产品名称', '物料类别']).agg({
            '物料成本': 'sum',
            '求和项:数量（箱）': 'sum',
            '客户代码': 'nunique'
        }).reset_index()

        material_usage.columns = ['产品代码', '产品名称', '物料类别', '物料总成本', '使用总量', '使用客户数']

        # 按物料成本或使用量排序
        material_usage = material_usage.sort_values('物料总成本', ascending=False).head(top_n)
        ...
        fig = px.bar(
            ...
            y='物料总成本',
            color='物料类别',
            title=f"TOP {top_n} 物料使用情况分析",
            labels={"产品名称": "物料名称", "物料总成本": "物料总成本 (元)"}
        )
        ...
        fig.update_layout(
            ...
            yaxis=dict(title="物料总成本 (元)"),
            margin=dict(l=40, r=40, t=50, b=180)  # 增加底部边距
        )

        st.plotly_chart(fig, use_container_width=True)

        st.info("说明：因缺少销售数据，此图仅展示物料使用情况，而非产出比。")
        st.markdown('</div>', unsafe_allow_html=True)
        return
    ...
    try:
        # ---------- 计算逻辑说明 ----------
        # 1. 按月份和产品代码计算各物料的成本总和
        # 2. 按月份计算各产品的销售额占比
        # 3. 基于产品类别特性估算各物料的营销效率系数
        # 4. 计算每种物料的投入产出比 = 分配销售额 / 物料成本
        # ----------------------------------
        ...
        # 合并销售数据
        material_analysis = pd.merge(material_specific_cost, monthly_sales_sum, on='月份名', how='left')
        ...
        material_analysis = pd.merge(material_analysis, material_month_total, on='月份名', how='left')
        # 防止除零错误
        material_analysis['成本占比'] = np.where(
            material_analysis['月度物料总成本'] > 0,
            material_analysis['物料成本'] / material_analysis['月度物料总成本'],
            0
        )

        # 使用合理的效率系数替代随机值，基于物料类别分配不同效率
        # 这里假设不同物料类别有不同的营销效率
        material_analysis['效率因子'] = 1.0  # 默认效率因子为1

        # 根据物料类别调整效率因子 - 使用业务逻辑而非随机值
        material_categories = {
            '促销物料': 1.2,  # 促销物料通常效率较高
            '陈列物料': 1.0,  # 陈列物料为标准效率
            '宣传物料': 0.9,  # 宣传物料通常是长期投资
            '赠品': 1.1,  # 赠品对短期销售有较好促进
            '包装物料': 0.8  # 包装物料通常是必要成本
        }

        for category, factor in material_categories.items():
            material_analysis.loc[
                material_analysis['物料类别'].str.contains(category, case=False, na=False), '效率因子'] = factor

        # 分配销售额 - 基于成本占比和效率因子
        # 物料销售额 = 月销售总额 × 该物料成本占比 × 效率因子
        material_analysis['分配销售额'] = material_analysis['销售金额'] * material_analysis['成本占比'] * \
                                          material_analysis['效率因子']

        # 计算物料产出比，确保不出现除零错误
        material_analysis['物料产出比'] = np.where(
            material_analysis['物料成本'] > 0,
            material_analysis['分配销售额'] / material_analysis['物料成本'],
            0
        )

        # 限制物料产出比的最大值，避免异常值
        max_roi = 10.0  # 设置产出比上限为10
        material_analysis['物料产出比'] = material_analysis['物料产出比'].clip(upper=max_roi)
        ...
        # 填充可能的NaN值
        material_roi['物料产出比'] = material_roi['物料产出比'].fillna(0)
        material_roi['使用总量'] = material_roi['使用总量'].fillna(0)
        material_roi['使用客户数'] = material_roi['使用客户数'].fillna(0)
        ...
        if len(filtered_material_roi) > 0:
            # 创建图表
            fig = px.bar(
            ...
            fig.update_traces(
                ...
                hovertemplate='<b>%{x}</b><br>' +
                              ...
                              '产品代码: %{customdata[2]}<br>' +
                              '使用客户数: %{customdata[0]} 家',
                customdata=np.column_stack((
            ...
            fig.add_annotation(
                ...
                text="物料产出比=1（盈亏平衡）",
                showarrow=False,
                font=dict(size=13, color="#FF3B30"),
                bgcolor="rgba(255,255,255,0.7)",
                borderpad=4
            )
            ...
            fig.update_layout(
                xaxis_title="物料名称",
                yaxis_title="平均物料产出比",
                margin=dict(l=40, r=40, t=50, b=240),  # 显著增加底部边距
                xaxis=dict(
                    tickangle=-90,  # 垂直显示名称
                    automargin=True  # 自动调整边距
                ),
            ...
            **物料产出比计算逻辑说明**:
            1. 物料产出比 = 分配销售额 ÷ 物料成本
            2. 分配销售额 = 月度销售总额 × 该物料成本占比 × 物料效率因子
            3. 物料效率因子基于物料类别设定: 促销物料(1.2), 陈列物料(1.0), 宣传物料(0.9), 赠品(1.1), 包装物料(0.8)
            4. 最终产出比值被限制在0-10之间，避免极端值
            """)
        else:
            st.info("没有足够的数据生成物料产出比分析图表。")
    except Exception as e:
        st.error(f"计算物料产出比时发生错误: {str(e)}")
        st.info("由于数据结构或计算逻辑问题，无法生成物料产出比分析。请检查数据格式和完整性。")
...
def validate_data_consistency(material_data, sales_data, distributor_data=None):
    ...
    # 检查共有的产品代码
    if 'product_consistency' not in st.session_state:
        material_products = set(material_data['产品代码'].unique()) if '产品代码' in material_data.columns else set()
        sales_products = set(sales_data['产品代码'].unique()) if '产品代码' in sales_data.columns else set()

        if material_products and sales_products:
            common_products = material_products.intersection(sales_products)
            if len(common_products) < min(len(material_products), len(sales_products)) * 0.5:
                warnings.append(
                    f"物料数据和销售数据的产品代码匹配度低于50%。物料数据有{len(material_products)}个产品，销售数据有{len(sales_products)}个产品，但共有产品只有{len(common_products)}个。")

        st.session_state['product_consistency'] = True  # 只检查一次
    ...
    if 'customer_consistency' not in st.session_state:
        ...
        if material_customers and sales_customers:
            ...
            if len(common_customers) < min(len(material_customers), len(sales_customers)) * 0.5:
                warnings.append(
                    f"物料数据和销售数据的客户代码匹配度低于50%。物料数据有{len(material_customers)}个客户，销售数据有{len(sales_customers)}个客户，但共有客户只有{len(common_customers)}个。")
...
def main():
    ...
    if consistency_warnings:
        with st.expander("⚠️ 数据一致性警告", expanded=True):
            for warning in consistency_warnings:
                st.warning(warning)

    # 应用全局字体样式调整 - 在创建任何UI元素之前应用
    set_global_font_styles(
        base_size=12,       # 基础字体大小
        title_size=18,      # 主标题大小
        subtitle_size=13,   # 副标题大小
        chart_title_size=14, # 图表标题大小
        text_size=11,       # 正文字体大小
        small_text_size=10  # 小字体大小
    )
