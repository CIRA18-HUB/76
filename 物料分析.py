...
def applicant_material_efficiency_analysis(filtered_material, filtered_sales):
    ...
    if not applicant_data.empty and len(applicant_data) >= 5:
        ...
        **图表解读：**
        ...
        # 选择要分析的申请人
        top_applicants_list = applicant_data.nlargest(10, '销售总额')['申请人'].tolist()
        selected_applicant = st.selectbox("选择要分析的申请人:", top_applicants_list)
        ...
        if selected_applicant:
            ...
            if not applicant_materials.empty:
                ...
                # 创建物料数量柱状图
                fig2 = px.bar(
                    material_usage.nlargest(10, '物料数量'),
                    x='物料名称',
                    y='物料数量',
                ...
                **特点总结:**
                - 该申请人最常使用的物料是: {material_usage.nlargest(1, '物料数量')['物料名称'].iloc[0]}
                - 物料成本占比最高的是: {material_usage.nlargest(1, '物料总成本')['物料名称'].iloc[0]}
