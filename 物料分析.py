...
def main():
        ...
        with tab2:
            ...
            if len(material_sales_relation) > 0:
                ...
                # 设置气泡大小 - 合理控制大小
                size_values = material_sales_relation['ROI'].clip(0.1, 10) * 1.8
            ...
            <div class="chart-explanation">
                ...
                背景区域划分了不同ROI的区域：低效区(ROI<1)、良好区(1≤ROI<2)和优秀区(ROI≥2)。</p>
            </div>
            ''', unsafe_allow_html=True)
            ...
            if len(material_roi) > 0:
                ...
                fig.update_traces(
                    ...
                    marker=dict(line=dict(width=0.8, color='white')),
                    hovertemplate='<b>%{x}</b><br>平均ROI: %{y:.2f}<br>物料类别: %{marker.color}<br>使用客户数: %{customdata[0]}<br>使用总量: %{customdata[1]}',
                    customdata=filtered_material_roi[['使用客户数', '使用总量']]
                )
            ...
            with col1:
                ...
                if len(distributor_cost_ratio) > 0:
                    ...
                    fig.add_trace(go.Bar(
                        ...
                        hovertemplate='<b>%{y}</b><br>物料费比: %{x:.2f}%<br>物料成本: ¥%{customdata[0]:,.2f}<br>销售总额: ¥%{customdata[1]:,.2f}<br>客户价值分层: %{customdata[2]}',
                        customdata=top_distributors[['物料总成本', '销售总额', '客户价值分层']]
                    ))
                ...
                if '所属区域' in filtered_distributor.columns:
                    ...
                    if len(region_cost_ratio) > 0:
                        ...
                        fig.add_trace(go.Bar(
                            ...
                            hovertemplate='<b>%{x}</b><br>平均费比: %{y:.2f}%<br>综合费比: %{customdata[0]:.2f}%<br>物料总成本: ¥%{customdata[1]:,.2f}<br>销售总额: ¥%{customdata[2]:,.2f}',
                            customdata=region_cost_ratio[['综合费比', '物料总成本', '销售总额']]
                        ))
                        ...
                        fig.add_trace(go.Scatter(
                            ...
                            yaxis='y2',
                            hovertemplate='<b>%{x}</b><br>经销商数量: %{y}<br>'
                        ))
            ...
            if len(monthly_data) > 0:
                ...
                fig.add_trace(
                    go.Bar(
                        ...
                        hovertemplate='<b>%{x}</b><br>物料成本: ¥%{y:,.2f}<br>销售金额: ¥%{customdata[0]:,.2f}<br>ROI: %{customdata[1]:.2f}<br>物料费比: %{customdata[2]:.2f}%',
                ...
                fig.add_trace(
                    go.Bar(
                        ...
                        hovertemplate='<b>%{x}</b><br>销售金额: ¥%{y:,.2f}<br>物料成本: ¥%{customdata[0]:,.2f}<br>ROI: %{customdata[1]:.2f}<br>物料费比: %{customdata[2]:.2f}%',
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        hovertemplate='<b>%{x}</b><br>ROI: %{y:.2f}<br>销售金额: ¥%{customdata[0]:,.2f}<br>物料成本: ¥%{customdata[1]:,.2f}',
                        customdata=monthly_data[['销售金额', '物料成本']]
                    ),
                    secondary_y=True
                )
