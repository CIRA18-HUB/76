...
def load_data(sample_data=False):
    ...
    if sample_data:
        ...
        try:
            # GitHub Raw URLs - 替换为您的GitHub仓库的原始文件链接
            github_base_url = "https://raw.githubusercontent.com/cira18-hub/76/main/"
            material_data_url = f"{github_base_url}2025物料源数据.xlsx"
            sales_data_url = f"{github_base_url}25物料源销售数据.xlsx"
            material_price_url = f"{github_base_url}物料单价.xlsx"
            ...
            # 下载并读取Excel文件
            material_data_resp = requests.get(material_data_url)
            sales_data_resp = requests.get(sales_data_url)
            material_price_resp = requests.get(material_price_url)

            material_data = pd.read_excel(io.BytesIO(material_data_resp.content))
            sales_data = pd.read_excel(io.BytesIO(sales_data_resp.content))
            material_price = pd.read_excel(io.BytesIO(material_price_resp.content))
