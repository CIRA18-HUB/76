...
def main():
        ...
        with tab2:
            ...
            # Filtro rápido para análisis más detallado
            ...
            if len(material_sales_relation) > 0:
                ...
                # 根据所选模式设置气泡大小
                if size_mode == "ROI值":
                    size_values = material_sales_relation['ROI'].clip(0.1, 10) * 5
                    size_column = 'ROI'
                elif size_mode == "物料多样性":
                    size_values = material_sales_relation['物料多样性'].clip(1, 30)
                    size_column = '物料多样性'
                else:  # "物料总成本"
                    # 对物料总成本进行归一化以适应气泡大小
                    min_cost = material_sales_relation['物料总成本'].min()
                    max_cost = material_sales_relation['物料总成本'].max()
                    normalized_cost = ((material_sales_relation['物料总成本'] - min_cost) / (
                                max_cost - min_cost) * 40) + 10
                    size_values = normalized_cost
                    size_column = '物料总成本'

                # 创建自定义悬停模板 - 大幅改进信息展示
                hover_template = """
                <b style='font-size:14px;'>%{hovertext}</b><br><br>
                <b style='color:#3B82F6;'>销售总额:</b> ¥%{y:,.2f}元<br>
                <b style='color:#10B981;'>物料总成本:</b> ¥%{x:,.2f}元<br>
                <b style='color:#F59E0B;'>ROI值:</b> %{customdata[0]:.2f}<br>
                <b style='color:#6366F1;'>物料销售比率:</b> %{customdata[1]:.2f}%<br>
                <b style='color:#EC4899;'>物料多样性:</b> %{customdata[2]}种<br>
                <hr style='margin:4px 0px;opacity:0.3'>
                <span style='font-size:10px;'>点击查看详情</span>
                """

                # 创建散点图 - 大幅优化悬停数据展示和视觉效果
                fig = go.Figure()

                # 为每个客户价值分层分别创建散点图，以便更好地控制视觉效果
                ...
                # ROI < 1 区域（低效）
                fig.add_trace(go.Scatter(
                    x=[min_cost, max_cost, max_cost, min_cost, min_cost],
                    y=[min_cost, max_cost, 0, 0, min_cost],
                    fill='toself',
                ...
                fig.add_trace(go.Scatter(
                    x=[min_cost, max_cost, max_cost, min_cost, min_cost],
                    y=[min_cost, max_cost, max_cost * 2, min_cost * 2, min_cost],
                    fill='toself',
                ...
                fig.add_trace(go.Scatter(
                    x=[min_cost, max_cost, max_cost, min_cost, min_cost],
                    y=[min_cost * 2, max_cost * 2, max_cost * 5, min_cost * 5, min_cost * 2],
                    fill='toself',
                ...
                # 大幅改进图表布局和格式 - 优化间距和可读性
                fig.update_layout(
                    height=600,  # 增加高度以改善可视化
                    title=None,
                    margin=dict(l=40, r=40, t=20, b=60),
                    paper_bgcolor='rgba(0,0,0,0)',  # 透明背景配合容器背景
                    plot_bgcolor='rgba(255,255,255,0.95)',  # 略微不透明的白色背景
                    font=dict(
                        family="PingFang SC, Helvetica Neue, Arial, sans-serif",
                        size=12,
                        color="#1F1F1F"
                    ...
                    legend=dict(
                        title=dict(
                            text="客户价值分层",
                            font=dict(size=13, color="#333", family="PingFang SC")
                        ),
                        orientation="h",
                        y=-0.15,
                        x=0.5,
                    ...
                    hoverdistance=100,  # 提高鼠标悬停的响应距离
                    hovermode="closest",
                    dragmode="zoom",  # 启用拖拽缩放功能
                    modebar=dict(
                        bgcolor="rgba(255,255,255,0.8)",
                        color="#333",
                        activecolor="#3B82F6"
                    ),
                    # 添加水印 (使用annotations替代shapes)
                    annotations=[
                        dict(
                            xref="paper",
                            yref="paper",
                            ...
                            font=dict(
                                family="PingFang SC",
                                size=80,
                                color="rgba(59, 130, 246, 0.04)"  # 使用rgba设置透明度
                            )
                        )
                    ]
                )
                ...
                fig.update_xaxes(
                    ...
                    griddash='dot',
                    showline=True,
                    linecolor='rgba(224, 228, 234, 0.8)',
                    linewidth=1.5,
                    tickprefix="¥",
                    tickformat=",d",
                    exponentformat="none",
                    ...
                    mirror=True,
                    type="log",  # 确保使用对数刻度
                    range=[np.log10(min_cost * 0.8), np.log10(max_cost * 1.2)],  # 设置适当的范围
                    showspikes=True,  # 添加鼠标指针辅助线
                ...
                fig.update_yaxes(
                    ...
                    mirror=True,
                    type="log",  # 确保使用对数刻度
                    range=[np.log10(min_cost * 0.8), np.log10(max_cost * 5)],  # 设置适当的范围，上限更高以显示高ROI区域
                    showspikes=True,  # 添加鼠标指针辅助线
                    spikecolor="rgba(59, 130, 246, 0.5)",
                    spikethickness=1,
                    spikedash="solid",
                    ...
                    updatemenus=[
                        dict(
                            ...
                            buttons=[
                                ...
                                dict(
                                    args=[{"visible": [
                                        i < 4 or fig.data[i]['name'] in ['高价值客户', 'ROI = 1 (盈亏平衡线)',
                                                                         'ROI = 2 (优秀水平)'] for i in
                                        range(len(fig.data))]}],
                                    label="仅高价值客户",
                                    method="update"
                                ...
                                dict(
                                    args=[{"visible": [
                                        i < 4 or fig.data[i]['name'] in ['成长型客户', 'ROI = 1 (盈亏平衡线)',
                                                                         'ROI = 2 (优秀水平)'] for i in
                                        range(len(fig.data))]}],
                                    label="仅成长型客户",
                                    method="update"
                                ...
                                dict(
                                    args=[{"visible": [
                                        i < 4 or fig.data[i]['name'] in ['稳定型客户', 'ROI = 1 (盈亏平衡线)',
                                                                         'ROI = 2 (优秀水平)'] for i in
                                        range(len(fig.data))]}],
                                    label="仅稳定型客户",
                                    method="update"
                                ...
                                dict(
                                    args=[{"visible": [
                                        i < 4 or fig.data[i]['name'] in ['低效型客户', 'ROI = 1 (盈亏平衡线)',
                                                                         'ROI = 2 (优秀水平)'] for i in
                                        range(len(fig.data))]}],
                                    label="仅低效型客户",
                                    method="update"
            ...
            <div class="chart-explanation">
                ...
                背景色区分了不同ROI的区域，从下到上：低效区(ROI<1)、良好区(1≤ROI<2)和优秀区(ROI≥2)。
                鼠标悬停可查看详细数据，上方提供过滤和可视化选项，右上角有交互控制。</p>
            </div>
            ''', unsafe_allow_html=True)
            ...
            with col1:
                ...
                if len(distributor_cost_ratio) > 0:
                    ...
                    fig.add_trace(go.Bar(
                        ...
                        hovertemplate='<b>%{y}</b><br>' +
                                      '物料费比: %{x:.2f}%<br>' +
                                      '销售总额: %{customdata[0]:,.2f}元<br>' +
                                      '物料总成本: %{customdata[1]:,.2f}元<br>' +
                                      '物料多样性: %{customdata[2]}<br>' +
                                      'ROI: %{customdata[3]:.2f}',
                        name='物料费比',
                        customdata=top_distributors[['销售总额', '物料总成本', '物料多样性', 'ROI']],
                        marker=dict(
                ...
                <div class="chart-explanation">
                    ...
                    条形长度表示费比大小，颜色代表客户价值分层。红色虚线是30%的行业参考线，低于这条线的经销商物料使用效率较好。
                    鼠标悬停可查看详细数据，包括销售额和ROI等指标。</p>
                </div>
                ''', unsafe_allow_html=True)
            ...
            with col2:
                    ...
                    if len(region_cost_ratio) > 0:
                        ...
                        fig.add_trace(go.Bar(
                            ...
                            hovertemplate='<b>%{x}</b><br>' +
                                          ...
                                          '经销商数量: %{customdata[1]}<br>' +
                                          '物料总成本: ¥%{customdata[2]:,.2f}<br>' +
                                          '销售总额: ¥%{customdata[3]:,.2f}',
                ...
                <div class="chart-explanation">
                    ...
                    鼠标悬停可查看详细数据，包括综合费比和销售总额等。可以看出哪些区域需要重点优化物料投入，为销售团队提供区域物料策略指导。</p>
                </div>
                ''', unsafe_allow_html=True)
            ...
            <div class="chart-explanation">
                ...
                当紫色线越高，说明投资回报率越好。鼠标悬停可查看详细数据，包括物料销售比率。
                通过观察趋势可以发现季节性规律，为物料的合理规划提供依据。</p>
            </div>
            ''', unsafe_allow_html=True)
            # ======= 经销商分析标签页 =======
            with tab3:
