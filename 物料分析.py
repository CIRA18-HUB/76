...
def main():
        ...
        with tab2:
            ...
            if len(material_sales_relation) > 0:
                ...
                # 改进图表布局和格式 - 调整边距解决文本重叠问题
                fig.update_layout(
                    height=580,  # 增加高度以提供更好的视觉效果和防止溢出
                    title=None,
                    margin=dict(l=40, r=40, t=20, b=40),  # 调整边距防止遮挡
                    paper_bgcolor='white',
                    plot_bgcolor='white',
                    font=dict(
                        family="PingFang SC, Helvetica Neue, Arial, sans-serif",
                ...
                fig.update_xaxes(
                    ...
                    ticks="outside",
                    ticklen=5,
                    tickfont=dict(size=11),  # 减小字体尺寸避免重叠
                    minor=dict(
                        showgrid=False
                    )
                )

                # 优化Y轴设置 - 修正货币单位问题并调整标签位置
                fig.update_yaxes(
                    title=dict(
                        text="销售收入 (人民币元) - 对数刻度",
                        font=dict(size=13, color="#333333"),
            ...
            with col1:
                ...
                if len(category_cost) > 0:
                    ...
                    fig.update_layout(
                        height=380,  # 增加高度
                        margin=dict(l=20, r=20, t=40, b=25),  # 调整底部边距
                        paper_bgcolor='white',
                        font=dict(
                            family="PingFang SC, Helvetica Neue, Arial, sans-serif",
                            size=12,
            ...
            with col1:
                ...
                if len(distributor_cost_ratio) > 0:
                    ...
                    fig.update_layout(
                        height=420,  # 增加高度以容纳所有标签
                        xaxis_title="物料费比(%)",
                        yaxis_title="",
                        margin=dict(l=180, r=50, t=40, b=30),  # 增加左侧边距，防止经销商名称遮挡
                        paper_bgcolor='white',
                        plot_bgcolor='white',
                        font=dict(
                            family="PingFang SC, Helvetica Neue, Arial, sans-serif",
                        ...
                        xaxis=dict(
                            ...
                            gridcolor='rgba(224, 228, 234, 0.4)',
                            gridwidth=0.5,
                            range=[0, max(top_distributors['物料销售比率']) * 1.3],  # 适当留出右侧空间
                            showline=True,
                            linecolor='#E0E4EA',
                            zeroline=True,
                            zerolinecolor='#E0E4EA',
            ...
            with col2:
                ...
                if len(region_cost_ratio) > 0:
                    ...
                    fig.update_layout(
                        ...
                        margin=dict(l=20, r=80, t=50, b=70),  # 调整边距，确保标签显示
                        paper_bgcolor='white',
                        plot_bgcolor='white',
                        font=dict(
                            family="PingFang SC, Helvetica Neue, Arial, sans-serif",
            ...
            if len(monthly_data) > 0:
                ...
                fig.update_layout(
                    height=450,  # 增加高度以容纳更多内容
                    barmode='group',
                    bargap=0.2,
                    bargroupgap=0.1,
                    xaxis_title="",
                    margin=dict(l=20, r=50, t=20, b=100),  # 显著增加底部边距，避免x轴标签被截断
                    legend=dict(
                        orientation="h",
                        y=1.15,  # 将图例移到更上方
                        x=0.5,
            ...
            try:
                ...
                if len(combo_df) > 0:
                    ...
                    fig.update_layout(
                        ...
                        xaxis_title="物料组合",
                        yaxis_title="平均ROI",
                        margin=dict(l=20, r=20, t=10, b=150),  # 显著增加底部边距，防止标签截断
                        paper_bgcolor='white',
                        plot_bgcolor='white',
                        font=dict(
                            family="PingFang SC, Helvetica Neue, Arial, sans-serif",
            ...
            try:
                ...
                if len(diversity_metrics) > 0:
                    ...
                    fig.update_layout(
                        title=None,
                        height=450,  # 增加高度
                        margin=dict(l=20, r=20, t=10, b=80),  # 增加底部边距
                        legend=dict(
                            orientation="h",
                            y=1.15,  # 移动图例到顶部
                            x=0.5,
