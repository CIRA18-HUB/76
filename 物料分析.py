        ...
        if '所属区域' in filtered_distributor.columns:
            ...
            if len(region_dist) > 0:
                ...
                for i in range(len(fig.data)):
                    fig.data[i].marker.line.width = 0.8
                    fig.data[i].marker.line.color = 'white'
                    fig.data[i].hovertemplate = '<b>%{x}</b><br>' + \
                                                '经销商数量: <b>%{y}</b><br>' + \
                    ...
                    # 确保customdata有正确的数据
                    fig.data[i].customdata = np.column_stack((
                        region_dist['平均ROI'],
                        region_dist['总销售额'],
                        region_dist['物料费比']
                    ))
