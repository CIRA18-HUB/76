...
def safe_data_processing(df, value_col, group_col=None, default_value=0):
    """
    安全地处理数据，处理NaN、空值和异常情况

    参数:
    - df: 要处理的数据框
    - value_col: 要处理的值列名
    - group_col: 分组列名(可选)
    - default_value: 默认值，用于填充NaN
    ...
    if value_col in processed_df.columns:
        processed_df[value_col] = processed_df[value_col].fillna(default_value)

    # 确保数据类型正确
    try:
        if value_col in processed_df.columns:
            if processed_df[value_col].dtype == 'object':
                # 尝试转换为数值型
    ...
    # 分组处理
    if group_col and group_col in processed_df.columns:
        # 确保分组列没有NaN
        processed_df[group_col] = processed_df[group_col].fillna('未知')
...
def configure_plotly_chart(fig, title, height=600, showlegend=True, legend_orientation="h"):
    ...
    fig.update_layout(
        ...
        margin=dict(l=100, r=100, t=120, b=100),  # 增加边距，确保标签有足够空间
        height=height,
        showlegend=showlegend,
        legend=dict(
            orientation=legend_orientation,
            yanchor="bottom" if legend_orientation == "h" else "top",
            y=1.10 if legend_orientation == "h" else 1,  # 进一步增加图例位置，防止与图表重叠
            ...
            for trace in fig.data:
                if trace.type == 'bar':
                    if hasattr(trace, 'textposition') and trace.textposition == 'outside':
                        if getattr(trace, 'orientation', '') == 'h':  # 水平条形图
    ...
    for i in range(len(fig.data)):
        if hasattr(fig.data[i], 'hovertemplate'):
            ...
            is_money_field = any(term in fig.data[i].name.lower() if hasattr(fig.data[i], 'name') else False
                                for term in ['销售', '成本', '价值', '额', 'sales', 'cost', 'value'])

            if is_money_field:
                if '%{y:,' in hover_text and '￥%{y:,' not in hover_text:
                    hover_text = hover_text.replace('%{y:,', '￥%{y:,')
                if '%{x:,' in hover_text and '￥%{x:,' not in hover_text and getattr(fig.data[i], 'orientation', '') == 'h':
                    hover_text = hover_text.replace('%{x:,', '￥%{x:,')
...
def create_chart_container(chart_figure, title, description, tips, insight_text=None):
    """
    创建带有标题、描述和解释的图表容器 - 简化版
    """
    # 对用户输入文本进行HTML转义，防止乱码
    safe_description = html.escape(description)
    safe_tips = [html.escape(tip) for tip in tips]
    safe_insight_text = html.escape(insight_text) if insight_text else None
    ...
    # 显示图表说明
    st.markdown(f"""
    <div style="background-color: #f8f9fe; border-radius: 10px; padding: 15px; margin-top: 15px; border: 1px solid #e2e8f0;">
        <h5 style="color: #1f3867; margin-bottom: 10px;">图表说明</h5>
        <p style="font-size: 0.9rem; color: #4a5568;">{safe_description}</p>

        <h5 style="color: #1f3867; margin-top: 15px; margin-bottom: 10px;">使用提示</h5>
        <ul style="padding-left: 20px; margin-bottom: 0;">
    """, unsafe_allow_html=True)

    # 单独添加每个提示项目
    for tip in safe_tips:
        st.markdown(f"<li style='font-size: 0.85rem; margin-bottom: 6px; color: #4a5568;'>{tip}</li>",
                    unsafe_allow_html=True)
    ...
    # 添加洞察文本（如果有）
    if safe_insight_text:
        st.markdown(f"""
        <div style="margin-top: 15px; background-color: rgba(45, 206, 137, 0.1); border-left: 4px solid #2dce89; padding: 12px; border-radius: 6px;">
            <h6 style="margin: 0 0 8px 0; font-size: 0.95rem; color: #276749;">数据洞察</h6>
            <p style="margin: 0; font-size: 0.9rem; color: #2d3748;">{safe_insight_text}</p>
        </div>
        """, unsafe_allow_html=True)
...
def calculate_fee_ratio(cost, sales):
    ...
    try:
        ...
        # 处理Series或DataFrame
            ...
            # 处理销售额中的0值和负值
            if isinstance(temp_sales, pd.DataFrame):
                mask = temp_sales <= 0
                if mask.any().any():
                    temp_sales = temp_sales.mask(mask, np.nan)
            else:  # Series
                mask = temp_sales <= 0
                if mask.any():
                    temp_sales = temp_sales.mask(mask, np.nan)
            ...
            # 处理成本中的负值
            if isinstance(temp_cost, pd.DataFrame):
                mask = temp_cost < 0
                if mask.any().any():
                    temp_cost = temp_cost.mask(mask, np.nan)
            else:  # Series
                mask = temp_cost < 0
                if mask.any():
                    temp_cost = temp_cost.mask(mask, np.nan)
...
def validate_data_columns(df, required_columns, dataframe_name="数据"):
    """
    验证DataFrame是否包含所需列，并处理缺失列

    参数:
    - df: 要验证的DataFrame
    - required_columns: 所需列名列表
    - dataframe_name: 用于错误消息的DataFrame名称

    返回:
    - 验证结果布尔值，验证通过的DataFrame（可能包含添加的空列）
    """
    if df is None or df.empty:
        st.warning(f"{dataframe_name}为空或不存在")
        return False, df
    ...
    if missing_columns:
        ...
        st.warning(f"{dataframe_name}缺少以下列: {missing_cols_str}")
        ...
        st.info(f"已为{dataframe_name}添加缺失的列，但其值为空。分析结果可能不完整或不准确。")
        return False, fixed_df
    ...
    with value_subtabs[1]:
        if not customer_value.empty:
            try:
                ...
                # 首先填充NaN值防止分群错误
                customer_value_temp = customer_value.copy()
                customer_value_temp['客户价值'] = customer_value_temp['客户价值'].fillna(0)
                customer_value_temp['物料效率'] = customer_value_temp['物料效率'].fillna(0)

                # 安全地创建分组
                if len(customer_value_temp) >= 4:
                    try:
                        customer_value['价值得分'] = pd.qcut(customer_value_temp['客户价值'], q=4,
                                                             labels=[1, 2, 3, 4]).astype(int)
                        customer_value['效率得分'] = pd.qcut(customer_value_temp['物料效率'], q=4,
                                                             labels=[1, 2, 3, 4]).astype(int)
                    except Exception as e:
                        # 如果qcut失败，使用简单的二分法
                        customer_value['价值得分'] = np.where(customer_value_temp['客户价值'] >=
                                                              customer_value_temp['客户价值'].median(), 3, 1)
                        customer_value['效率得分'] = np.where(customer_value_temp['物料效率'] >=
                                                              customer_value_temp['物料效率'].median(), 3, 1)
                else:
                    # 数据不足以分组时使用简单分配
                    customer_value['价值得分'] = 2
                    customer_value['效率得分'] = 2
    ...
    with value_subtabs[2]:
        if not customer_value.empty:
            try:
                ...
                # 修改这里：放宽筛选条件
                # 原始条件: (customer_value['潜力得分'] > 60) & (customer_value['销售总额'] < sales_threshold)
                # 计算潜力得分平均值
                potential_score_avg = customer_value['潜力得分'].mean()

                # 使用放宽的筛选条件 - 只需大于平均得分的80%即可
                potential_customers = customer_value[customer_value['潜力得分'] > potential_score_avg * 0.8]

                # 如果结果仍然太少，直接取潜力得分最高的前5名
                if len(potential_customers) < 5:
                    potential_customers = customer_value.nlargest(5, '潜力得分')
...
def render_region_geographic_combined_tab(filtered_material, filtered_sales):
    """
    整合区域性能分析和地理分布可视化的选项卡
    """
    st.markdown("<h2 class='section-header'>区域与地理分析</h2>", unsafe_allow_html=True)
...
def render_time_fee_ratio_combined_tab(filtered_material, filtered_sales, overall_cost_sales_ratio):
    """
    整合时间趋势分析和费比分析的选项卡
    """
    st.markdown("<h2 class='section-header'>时间与费比分析</h2>", unsafe_allow_html=True)
    ...
    with subtabs[0]:
        ...
        try:
            ...
            st.error(f"生成销售与成本趋势分析时出错: {str(e)}")
            import traceback
            st.error(f"详细错误: {traceback.format_exc()}")

    # 费比趋势子选项卡
    with subtabs[1]:
        # 整合自原时间趋势分析和费比分析选项卡，保留更优的图表
        try:
            # 按月份聚合物料和销售数据
            ...
            if not monthly_metrics.empty and len(monthly_metrics) >= 3:
                ...
                fig_fee_ratio.add_trace(
                    go.Bar(
                        x=monthly_metrics['月份'],
                        y=monthly_metrics['费比'],
                        name='费比',
                        marker_color='#11cdef',
                        hovertemplate='月份: %{x}<br>费比: %{y:.2f}%<br><extra></extra>'
                    )
                ...
                # 计算三个月移动平均
                monthly_metrics['费比_3MA'] = monthly_metrics['费比'].rolling(window=3, min_periods=1).mean()

                # 添加费比移动平均线
                fig_fee_ratio.add_trace(
                    go.Scatter(
                        x=monthly_metrics['月份'],
                        y=monthly_metrics['费比_3MA'],
                        mode='lines+markers',
                        name='费比 (3月移动平均)',
                        line=dict(color='#f5365c', width=3),
                        marker=dict(size=8, symbol='circle', color='#f5365c', line=dict(width=1, color='white')),
                ...
                # 添加参考线 - 平均费比
                avg_fee_ratio = monthly_metrics['费比'].mean()
                fig_fee_ratio.add_shape(
                    type='line',
                    x0=0,
                    y0=avg_fee_ratio,
                    x1=len(monthly_metrics['月份']) - 1,
                    y1=avg_fee_ratio,
                    line=dict(
                        color='#2dce89',
                        width=2,
                ...
                monthly_metrics['费比环比变化率'] = monthly_metrics['费比环比变化'] / monthly_metrics['费比'].shift(
                    1) * 100
    ...
    with subtabs[3]:
        try:
            ...
            # 使用1.5倍平均费比作为阈值，或更保守的1.5倍全局平均费比
            anomaly_threshold = max(overall_cost_sales_ratio * 1.5, customer_metrics['费比'].mean() * 1.5)
            anomalies = customer_metrics[customer_metrics['费比'] > anomaly_threshold]

            # 显示费比异常警告卡片
            display_fee_ratio_anomalies(anomalies, overall_cost_sales_ratio)
        except Exception as e:
            st.error(f"生成费比异常分析时出错: {str(e)}")
            import traceback
            st.error(f"详细错误: {traceback.format_exc()}")


def render_material_roi_combined_tab(filtered_material, filtered_sales):
    """
    整合物料效益分析和利润最大化分析的选项卡
    """
    st.markdown("<h2 class='section-header'>物料投资回报分析</h2>", unsafe_allow_html=True)
    ...
    with subtabs[1]:
        ...
        with subtabs[2]:
            try:
                # 显示最优物料分配建议
                if 'material_roi' in locals() and not material_roi.empty:
                    display_optimal_material_allocation(material_roi, filtered_material, filtered_sales)
                else:
                    st.warning("没有足够的数据来生成最优物料分配建议。")
            except Exception as e:
                st.error(f"生成物料投资优化建议时出错: {str(e)}")
                import traceback
                st.error(f"详细错误: {traceback.format_exc()}")
        ...
        with subtabs[3]:
            try:
                ...
                if not valid_customers.empty and len(valid_customers) >= 6:
                    ...
                    with cols[0]:
                        st.markdown("""
                                    <div style="background-color: rgba(76, 175, 80, 0.1); border-radius: 10px; padding: 15px; border: 1px solid #4CAF50;">
                                        <h4 style="color: #2E7D32; margin-top: 0;">高效益客户特点</h4>
                                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                                            <div style="background-color: white; padding: 10px; border-radius: 5px;">
                                                <div style="font-size: 0.8rem; color: #555;">物料数量</div>
                                                <div style="font-size: 1.2rem; font-weight: bold;">{}</div>
                                            </div>
                                            <div style="background-color: white; padding: 10px; border-radius: 5px;">
                                                <div style="font-size: 0.8rem; color: #555;">物料总成本</div>
                                                <div style="font-size: 1.2rem; font-weight: bold;">{}</div>
                                            </div>
                                        </div>
                                        <div style="margin-top: 10px; background-color: white; padding: 10px; border-radius: 5px;">
                                            <div style="font-size: 0.8rem; color: #555;">销售总额</div>
                                            <div style="font-size: 1.2rem; font-weight: bold;">{}</div>
                                        </div>
                                        <div style="margin-top: 10px; background-color: white; padding: 10px; border-radius: 5px;">
                                            <div style="font-size: 0.8rem; color: #555;">物料效益</div>
                                            <div style="font-size: 1.2rem; font-weight: bold; color: #2E7D32;">{}</div>
                                        </div>
                                    </div>
                                    """.format(
                            high_avg['物料数量'].astype(int),
                            f"￥{high_avg['物料总成本']:,.0f}",
                            f"￥{high_avg['销售总额']:,.0f}",
                            f"￥{high_avg['物料效益']:,.2f}/件"
                        ), unsafe_allow_html=True)

                    with cols[1]:
                        st.markdown("""
                                    <div style="background-color: rgba(244, 67, 54, 0.1); border-radius: 10px; padding: 15px; border: 1px solid #F44336;">
                                        <h4 style="color: #C62828; margin-top: 0;">低效益客户特点</h4>
                                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                                            <div style="background-color: white; padding: 10px; border-radius: 5px;">
                                                <div style="font-size: 0.8rem; color: #555;">物料数量</div>
                                                <div style="font-size: 1.2rem; font-weight: bold;">{}</div>
                                        ...
                                        <div style="margin-top: 10px; background-color: white; padding: 10px; border-radius: 5px;">
                                            <div style="font-size: 0.8rem; color: #555;">物料效益</div>
                                            <div style="font-size: 1.2rem; font-weight: bold; color: #C62828;">{}</div>
                                        </div>
                                    </div>
                                    """.format(
                            low_avg['物料数量'].astype(int),
                            f"￥{low_avg['物料总成本']:,.0f}",
                            f"￥{low_avg['销售总额']:,.0f}",
                            f"￥{low_avg['物料效益']:,.2f}/件"
                        ), unsafe_allow_html=True)
                    ...
                    # 差异与提升倍数
                    st.markdown("""
                                <div style="background-color: rgba(33, 150, 243, 0.1); border-radius: 10px; padding: 15px; margin-top: 15px; border: 1px solid #2196F3;">
                                    <h4 style="color: #1565C0; margin-top: 0;">两组客户差异分析</h4>
                                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;">
                                        <div style="background-color: white; padding: 12px; border-radius: 8px;">
                                            <div style="font-size: 0.9rem; color: #555; margin-bottom: 8px;">销售额差异</div>
                                            <div style="font-size: 1.3rem; font-weight: bold; color: #1565C0;">{}</div>
                                            <div style="font-size: 0.8rem; color: #1565C0; margin-top: 5px;">{} 倍</div>
                                        </div>
                                        <div style="background-color: white; padding: 12px; border-radius: 8px;">
                                            <div style="font-size: 0.9rem; color: #555; margin-bottom: 8px;">物料效益差异</div>
                                            <div style="font-size: 1.3rem; font-weight: bold; color: #1565C0;">{}</div>
                                            <div style="font-size: 0.8rem; color: #1565C0; margin-top: 5px;">{} 倍</div>
                                        </div>
                                    </div>
                ...
                st.error(f"生成物料效益对比分析时出错: {str(e)}")
                import traceback
                st.error(f"详细错误: {traceback.format_exc()}")
...
def main():
    ...
    # 在各选项卡中渲染内容
    with tab1:
        render_region_geographic_combined_tab(filtered_material, filtered_sales)

    with tab2:
        render_time_fee_ratio_combined_tab(filtered_material, filtered_sales, overall_cost_sales_ratio)

    with tab3:
        render_customer_value_tab(filtered_material, filtered_sales)

    with tab4:
        render_material_roi_combined_tab(filtered_material, filtered_sales)
    ...
    <div style="margin-top: 50px; padding-top: 20px; border-top: 1px solid #eee; text-align: center; color: #666; font-size: 0.8rem;">
        <p>口力营销物料与销售分析仪表盘 | 版本 1.0.1 | 最后更新: 2025年4月</p>
        <p>使用Streamlit和Plotly构建 | 数据更新频率: 每月</p>
    </div>
    """, unsafe_allow_html=True)

    # 显示帮助信息（可折叠）
    with st.expander("💡 使用帮助与提示"):
        st.markdown("""
        ## 使用指南

        ### 基本操作
        - **数据筛选**: 使用左侧面板的筛选器选择区域、省份和时间范围
        - **图表交互**: 大多数图表支持悬停查看详情、缩放和下载
        - **指标解释**: 将鼠标悬停在各指标上可查看详细说明

        ### 重要指标说明
        - **费比**: 物料成本占销售额的百分比，越低越好
        - **物料效率**: 每件物料产生的销售额，越高越好
        - **ROI**: 投资回报率，销售额除以物料成本，越高越好

        ### 常见问题
        1. **为什么有些图表显示"没有足够数据"?**
           - 当前筛选条件下没有满足图表所需的数据，尝试放宽筛选条件

        2. **如何找出物料使用效率最高的客户?**
           - 在"客户价值分析"选项卡查看客户分群，或在"物料投资回报分析"选项卡查看物料效益对比

        3. **如何优化物料投放策略?**
           - 参考"物料投资回报分析"选项卡中的物料ROI矩阵和物料投资优化建议
        """)
