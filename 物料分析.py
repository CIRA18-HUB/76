...
def main():
        ...
        with tab2:
            ...
            <div class="chart-explanation">
                <div class="chart-explanation-title">图表解读：</div>
                <p>这个散点图展示了物料投入和销售产出的关系。每个点代表一个经销商。横轴是投入物料成本，纵轴是获得的销售额，单位为人民币元。点越大表示ROI越高。红色虚线是盈亏平衡线(ROI=1)，绿色虚线是优秀水平线(ROI=2)。鼠标悬停可查看详细数据包括物料销售比率和物料多样性。</p>
            </div>
            ''', unsafe_allow_html=True)
            ...
            if len(material_roi) > 0:
                ...
                fig.update_layout(
                    xaxis_title="物料名称",
                    yaxis_title="平均ROI",
                    margin=dict(l=20, r=20, t=40, b=230),  # 大幅增加底部边距，确保标签完全显示
                    paper_bgcolor='white',
                    plot_bgcolor='white',
                    font=dict(
                        family="PingFang SC, Helvetica Neue, Arial, sans-serif",
            ...
            <div class="chart-explanation">
                <div class="chart-explanation-title">图表解读：</div>
                <p>这个柱状图显示了TOP物料的平均ROI(投资回报率)，可通过上方选择器调整显示数量。柱子越高表示该物料带来的回报越高，不同颜色代表不同物料类别。红色虚线是ROI=1的参考线，低于这条线的物料是亏损的。鼠标悬停可查看详细数据，包括物料代码、使用总量和使用客户数。</p>
            </div>
            ''', unsafe_allow_html=True)
            ...
            with col1:
                ...
                if len(distributor_cost_ratio) > 0:
                    ...
                    fig.update_layout(
                        ...
                        xaxis_title="物料费比(%)",
                        yaxis_title="",
                        margin=dict(l=180, r=50, t=40, b=30),
                        paper_bgcolor='white',
                        plot_bgcolor='white',
                        font=dict(
                            family="PingFang SC, Helvetica Neue, Arial, sans-serif",
                ...
                <div class="chart-explanation">
                    <div class="chart-explanation-title">图表解读：</div>
                    <p>此图展示了TOP 10经销商的物料费比(物料成本占销售额的百分比)。可切换按销售额或费比排序。柱子越短表示费比越低，物料使用效率越高。红色虚线是30%的行业参考线，低于这条线的经销商物料使用效率较好。鼠标悬停可查看详细数据，包括销售额和ROI等指标。</p>
                </div>
                ''', unsafe_allow_html=True)
            ...
            with col2:
                ...
                if '所属区域' in filtered_distributor.columns:
                    ...
                    if len(region_cost_ratio) > 0:
                        ...
                        fig.add_trace(go.Bar(
                            ...
                            textposition='outside',
                            marker=dict(line=dict(width=0.5, color='white')),
                            hovertemplate='<b>%{x}</b><br>平均费比: %{y:.2f}%<br>综合费比: %{customdata[0]:.2f}%<br>经销商数量: %{customdata[1]}<br>物料总成本: ¥%{customdata[2]:,.2f}<br>销售总额: ¥%{customdata[3]:,.2f}',
                            customdata=region_cost_ratio[['综合费比', '经销商数量', '物料总成本', '销售总额']]
                        ))
                        ...
                        fig.update_layout(
                            ...
                            margin=dict(l=20, r=80, t=50, b=70),
                            paper_bgcolor='white',
                            plot_bgcolor='white',
                            font=dict(
                                family="PingFang SC, Helvetica Neue, Arial, sans-serif",
                ...
                <div class="chart-explanation">
                    <div class="chart-explanation-title">图表解读：</div>
                    <p>此图展示了各区域的平均物料费比(蓝色柱)和经销商数量(黄线)。红色虚线是30%的参考线，柱子低于此线表示区域物料使用效率较好。鼠标悬停可查看详细数据，包括综合费比和销售总额等。可以看出哪些区域需要重点优化物料投入，为销售团队提供区域物料策略指导。</p>
                </div>
                ''', unsafe_allow_html=True)
            ...
            if len(monthly_data) > 0:
                ...
                fig.add_trace(
                    go.Bar(
                        ...
                        marker=dict(line=dict(width=0.5, color='white')),
                        hovertemplate='<b>%{x}</b><br>物料成本: ¥%{y:,.2f}<br>销售金额: ¥%{customdata[0]:,.2f}<br>ROI: %{customdata[1]:.2f}<br>物料销售比率: %{customdata[2]:.2f}%',
                ...
                fig.add_trace(
                    go.Bar(
                        ...
                        marker=dict(line=dict(width=0.5, color='white')),
                        hovertemplate='<b>%{x}</b><br>销售金额: ¥%{y:,.2f}<br>物料成本: ¥%{customdata[0]:,.2f}<br>ROI: %{customdata[1]:.2f}<br>物料销售比率: %{customdata[2]:.2f}%',
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        hovertemplate='<b>%{x}</b><br>ROI: %{y:.2f}<br>销售金额: ¥%{customdata[0]:,.2f}<br>物料成本: ¥%{customdata[1]:,.2f}<br>物料销售比率: %{customdata[2]:.2f}%',
            ...
            <div class="chart-explanation">
                <div class="chart-explanation-title">图表解读：</div>
                <p>这个图表展示了每月的物料成本(蓝色柱子)、销售额(绿色柱子)和ROI(紫色线)。当绿色柱子明显高于蓝色柱子时，说明效率好；当紫色线越高，说明投资回报率越好。鼠标悬停可查看详细数据，包括物料销售比率。通过观察趋势可以发现季节性规律，为物料的合理规划提供依据。</p>
            </div>
            ''', unsafe_allow_html=True)
            # ======= 经销商分析标签页 =======
        with tab3:
