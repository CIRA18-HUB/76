...
def process_data(material_data, sales_data, material_price):
    ...
    def create_distributor_analysis_tab(filtered_distributor, material_data, sales_data):
        ...
        Features:
        - Enhanced hover functionality for detailed distributor data viewing
        - Interactive visualizations with rich tooltips
        - Detailed distributor performance metrics and material usage patterns
        - Comparative analysis between high and low performing distributors
        - Visual insights on material combination effectiveness
        """
        ...
        with col1:
            ...
            if len(filtered_distributor) > 0:
                ...
                # 添加水平条形图 - ROI值 (增强版悬停数据)
                # 准备更丰富的自定义数据
                hover_data = np.column_stack((
                    efficient_distributors['销售总额'],
                    efficient_distributors['物料总成本'],
                    efficient_distributors['物料销售比率'],
                    efficient_distributors['客户价值分层'],
                    efficient_distributors[
                        '物料多样性'] if '物料多样性' in efficient_distributors.columns else np.zeros(
                        len(efficient_distributors)),
                    efficient_distributors[
                        '所属区域'] if '所属区域' in efficient_distributors.columns else np.array(
                        ['未知'] * len(efficient_distributors)),
                    efficient_distributors['省份'] if '省份' in efficient_distributors.columns else np.array(
                        ['未知'] * len(efficient_distributors)),
                    efficient_distributors['客户代码']
                ))
        ...
        with col2:
            ...
            if len(filtered_distributor) > 0:
                ...
                # 准备更丰富的自定义数据
                hover_data = np.column_stack((
                    inefficient_distributors['销售总额'],
                    inefficient_distributors['物料总成本'],
                    inefficient_distributors['物料销售比率'],
                    inefficient_distributors['客户价值分层'],
                    inefficient_distributors[
                        '物料多样性'] if '物料多样性' in inefficient_distributors.columns else np.zeros(
                        len(inefficient_distributors)),
                    inefficient_distributors[
                        '所属区域'] if '所属区域' in inefficient_distributors.columns else np.array(
                        ['未知'] * len(inefficient_distributors)),
                    inefficient_distributors[
                        '省份'] if '省份' in inefficient_distributors.columns else np.array(
                        ['未知'] * len(inefficient_distributors)),
                    inefficient_distributors['客户代码']
                ))
        ...
        # ====================
        # 经销商ROI分布与物料使用分析 - Distributor ROI Distribution & Material Usage Section
        # ====================
        st.markdown(
        ...
        with col2:
            ...
            if len(filtered_distributor) > 0:
                ...
                extra_hover_data = {
                    ...
                    '物料销售比率': ':.2f%',
                    '客户代码': True,
                    '所属区域': True if '所属区域' in filtered_distributor.columns else False,
                    '省份': True if '省份' in filtered_distributor.columns else False,
                    '销售人员': True if '销售人员' in filtered_distributor.columns else False
                }

                # 添加额外的计算字段（如果有）
                if '销售总额' in filtered_distributor.columns and '物料多样性' in filtered_distributor.columns:
                    filtered_distributor['销售效率'] = filtered_distributor['销售总额'] / filtered_distributor[
                        '物料多样性']
                    extra_hover_data['销售效率'] = ':,.2f元/种'
                ...
                fig = px.scatter(
                    ...
                    custom_data=['客户代码']  # 添加客户代码用于潜在的点击交互
                )
        ...
        # ====================
        # 高低效经销商物料组合对比 - High/Low Efficiency Distributors Material Usage Comparison
        # ====================
        st.markdown('<div class="feishu-chart-title" style="margin-top: 30px;">高低效经销商物料组合对比</div>',
                    unsafe_allow_html=True)
        ...
        if len(filtered_distributor) > 0:
            ...
            for _, dist in high_eff_dist.iterrows():
                dist_code = dist['客户代码']
                dist_products = material_data[material_data['客户代码'] == dist_code]

                if len(dist_products) > 0:
                    # 获取具体物料名称及成本
                    products = dist_products.groupby('产品名称')['物料成本'].sum().reset_index()
                    products = products.sort_values('物料成本', ascending=False).head(5)  # 取成本最高的5种物料
                    products['经销商名称'] = dist['经销商名称']
                    products['ROI'] = dist['ROI']
                    products['占比'] = products['物料成本'] / products['物料成本'].sum() * 100
                    high_eff_products.append(products)
            ...
            for _, dist in low_eff_dist.iterrows():
                ...
                if len(dist_products) > 0:
                    ...
                    products['ROI'] = dist['ROI']
                    products['占比'] = products['物料成本'] / products['物料成本'].sum() * 100
                    low_eff_products.append(products)

            # 合并数据
            high_eff_df = pd.concat(high_eff_products) if high_eff_products else pd.DataFrame()
            low_eff_df = pd.concat(low_eff_products) if low_eff_products else pd.DataFrame()

            # 分析高效经销商最常用的物料组合
            high_eff_combos = {}
            if not high_eff_df.empty:
                for _, row in high_eff_df.iterrows():
                    product = row['产品名称']
                    for other_row in high_eff_df.iterrows():
                        if other_row[1]['产品名称'] != product and other_row[1]['经销商名称'] == row[
                            '经销商名称']:
                            combo = tuple(sorted([product, other_row[1]['产品名称']]))
                            if combo in high_eff_combos:
                                high_eff_combos[combo] += 1
                            else:
                                high_eff_combos[combo] = 1
            ...
            if not low_eff_df.empty:
                for _, row in low_eff_df.iterrows():
                    product = row['产品名称']
                    for other_row in low_eff_df.iterrows():
                        if other_row[1]['产品名称'] != product and other_row[1]['经销商名称'] == row[
                            '经销商名称']:
                            combo = tuple(sorted([product, other_row[1]['产品名称']]))
                            if combo in low_eff_combos:
                                low_eff_combos[combo] += 1
                            else:
                                low_eff_combos[combo] = 1

            # 将组合转换为DataFrame
            high_combo_df = pd.DataFrame([
                {'物料组合': ' + '.join(combo), '出现次数': count}
                for combo, count in sorted(high_eff_combos.items(), key=lambda x: x[1], reverse=True)
            ])

            low_combo_df = pd.DataFrame([
                {'物料组合': ' + '.join(combo), '出现次数': count}
                for combo, count in sorted(low_eff_combos.items(), key=lambda x: x[1], reverse=True)
            ])

            # 添加占比计算
            if not high_combo_df.empty:
                high_combo_df['占比'] = high_combo_df['出现次数'] / high_combo_df['出现次数'].sum() * 100

            if not low_combo_df.empty:
                low_combo_df['占比'] = low_combo_df['出现次数'] / low_combo_df['出现次数'].sum() * 100

            # 显示分析结果
            col1, col2 = st.columns(2)

            with col1:
                st.markdown('<div class="feishu-chart-container" style="padding: 24px; margin-bottom: 16px;">',
                            unsafe_allow_html=True)

                if not high_combo_df.empty:
                    st.markdown(
                        '<div style="font-weight: 600; color: #0FC86F; margin-bottom: 10px; font-size: 16px;">高效经销商常用物料组合</div>',
                        unsafe_allow_html=True)

                    # 取前5个组合
                    top_combos = high_combo_df.head(5)

                    # 创建水平条形图
                    fig = px.bar(
                        top_combos.sort_values('占比', ascending=True),
                        y='物料组合',
                        x='占比',
                        orientation='h',
                        color='占比',
            ...
            with col2:
                ...
                if not low_combo_df.empty:
                    st.markdown(
                        '<div style="font-weight: 600; color: #F53F3F; margin-bottom: 10px; font-size: 16px;">低效经销商常用物料组合</div>',
                        unsafe_allow_html=True)

                    # 取前5个组合
                    top_combos = low_combo_df.head(5)
            ...
            # 组合对比分析
            st.markdown('<div class="feishu-chart-container" style="margin-top: 20px;">',
                        unsafe_allow_html=True)

            if not high_combo_df.empty and not low_combo_df.empty:
                # 创建双组合对比表
                st.markdown('''
                <div style="padding: 10px 0;">
                    <div style="font-weight: 600; font-size: 16px; margin-bottom: 15px; text-align: center;">高效vs低效经销商物料组合比较</div>
                    <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
                        <thead>
                            <tr style="background-color: #F5F7FA;">
                                <th style="padding: 12px 15px; border-bottom: 1px solid #E0E4EA; text-align: left; width: 50%;">高效经销商常用组合</th>
                                <th style="padding: 12px 15px; border-bottom: 1px solid #E0E4EA; text-align: left; width: 50%;">低效经销商常用组合</th>
                            </tr>
                        </thead>
                        <tbody>
                ''', unsafe_allow_html=True)
                ...
                # 获取前5个组合
                high_top5 = high_combo_df.head(5)
                low_top5 = low_combo_df.head(5)

                # 确保有5行对比
                max_rows = max(len(high_top5), len(low_top5))
                for i in range(max_rows):
                    high_combo = high_top5.iloc[i]['物料组合'] if i < len(high_top5) else "-"
                    high_pct = f"{high_top5.iloc[i]['占比']:.1f}%" if i < len(high_top5) else "-"
                    low_combo = low_top5.iloc[i]['物料组合'] if i < len(low_top5) else "-"
                    low_pct = f"{low_top5.iloc[i]['占比']:.1f}%" if i < len(low_top5) else "-"

                    st.markdown(f'''
                    <tr>
                        <td style="padding: 12px 15px; border-bottom: 1px solid #E0E4EA;">
                            <div style="display: flex; justify-content: space-between;">
                                <span>{high_combo}</span>
                                <span style="color: #0FC86F; font-weight: 500;">{high_pct}</span>
                            </div>
                        </td>
                        <td style="padding: 12px 15px; border-bottom: 1px solid #E0E4EA;">
                            <div style="display: flex; justify-content: space-between;">
                                <span>{low_combo}</span>
                                <span style="color: #F53F3F; font-weight: 500;">{low_pct}</span>
                            </div>
                        </td>
                    </tr>
                    ''', unsafe_allow_html=True)
                ...
                # 获取高效组合中的高频物料
                high_products = set()
                for combo in high_combo_df['物料组合']:
                    for prod in combo.split(' + '):
                        high_products.add(prod)

                # 获取低效组合中的高频物料
                low_products = set()
                for combo in low_combo_df['物料组合']:
                    for prod in combo.split(' + '):
                        low_products.add(prod)

                # 分析差异物料
                unique_to_high = high_products - low_products
                unique_to_low = low_products - high_products

                # 显示物料差异洞察
                st.markdown(f'''
                <div style="background-color: #F5FAFF; border-radius: 8px; padding: 15px; margin-top: 20px;">
                    <div style="font-weight: 600; color: #2B5AED; margin-bottom: 10px;">物料组合关键差异洞察</div>
                    <p style="margin: 0 0 10px 0; line-height: 1.6;">
                        <span style="font-weight: 500;">高效经销商独有物料：</span> 
                        <span style="color: #0FC86F;">{', '.join(list(unique_to_high)[:3]) if unique_to_high else '无明显差异'}</span>
                    </p>
                    <p style="margin: 0 0 10px 0; line-height: 1.6;">
                        <span style="font-weight: 500;">低效经销商独有物料：</span> 
                        <span style="color: #F53F3F;">{', '.join(list(unique_to_low)[:3]) if unique_to_low else '无明显差异'}</span>
                    </p>
                    <p style="margin: 0; line-height: 1.6;">
                        分析显示，高效经销商更注重物料间的搭配使用，形成协同效应。建议低效经销商学习高效组合模式，
                        适当增加多样性，注重物料间的互补性，避免过度依赖单一类型物料。
                    </p>
                </div>
                ''', unsafe_allow_html=True)
            else:
                st.info("暂无足够数据进行物料组合对比分析")

            st.markdown('</div>', unsafe_allow_html=True)
        else:
            st.info("暂无经销商数据，无法进行物料组合分析。")

        # ====================
        # 高效经销商最佳实践 - High Efficiency Distributor Best Practices
        # ====================
        ...
        if len(top_roi_distributors) > 0:
            st.markdown(
                '<div class="feishu-grid" style="display: flex; justify-content: space-between; gap: 16px;">',
                unsafe_allow_html=True)
            ...
            for i, (_, distributor) in enumerate(top_roi_distributors.iterrows()):
                ...
                # 获取该经销商的物料使用数据
                dist_materials = material_data[material_data['客户代码'] == distributor_code]
                ...
                # 增加卡片宽度，确保内容不被截断
                # 提取额外的经销商数据用于详细信息卡
                district = distributor['所属区域'] if '所属区域' in distributor and not pd.isna(
                    distributor['所属区域']) else "未知"
                province = distributor['省份'] if '省份' in distributor and not pd.isna(
                    distributor['省份']) else "未知"
                sales_person = distributor['销售人员'] if '销售人员' in distributor and not pd.isna(
                    distributor['销售人员']) else "未知"
                material_ratio = distributor['物料销售比率'] if '物料销售比率' in distributor and not pd.isna(
                    distributor['物料销售比率']) else 0

                # 创建带悬停详情的经销商卡片
                st.markdown(f'''
                <div class="distributor-card" 
                    style="flex: 1; background-color: white; border-radius: 12px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.08); 
                           padding: 20px; min-height: 180px; margin-bottom: 16px; overflow-wrap: break-word; position: relative;
                           transition: all 0.2s ease;"
                    onmouseover="showDetails_{i}()" 
                    onmouseout="hideDetails_{i}()">

                    <!-- 基础信息 -->
                    <div style="font-size: 16px; font-weight: 600; margin-bottom: 10px; color: #1F1F1F;">{distributor_name}</div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 10px;">
                        <div><span style="font-weight: 500; color: #646A73;">ROI:</span> <span style="font-weight: 600; color: #0FC86F;">{distributor_roi:.2f}</span></div>
                    ...
                    <div style="margin-top: 12px;">
                        <span style="display: inline-block; padding: 3px 8px; background-color: rgba(15, 200, 111, 0.1); color: #0FC86F; border-radius: 4px; font-size: 12px; font-weight: 500; margin-right: 6px;">最佳实践</span>
                        <span style="display: inline-block; padding: 3px 8px; background-color: rgba(43, 90, 237, 0.1); color: #2B5AED; border-radius: 4px; font-size: 12px; font-weight: 500; margin-right: 6px;">物料搭配优</span>
                        <span style="display: inline-block; padding: 3px 8px; background-color: rgba(255, 170, 0, 0.1); color: #FFAA00; border-radius: 4px; font-size: 12px; font-weight: 500;">可复制模式</span>
                    </div>

                    <!-- 悬停时显示的详细信息 -->
                    <div id="details_{i}" style="display: none; position: absolute; top: 0; left: 0; width: 100%; height: 100%; 
                          background-color: rgba(255,255,255,0.98); border-radius: 12px; padding: 20px; z-index: 10;">
                        <div style="font-size: 16px; font-weight: 600; margin-bottom: 15px; color: #2B5AED;">{distributor_name} <span style="font-size: 12px; color: #666; font-weight: normal;">详细数据</span></div>

                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 12px;">
                            <div><span style="color: #666; font-size: 12px;">客户代码</span><br><b>{distributor_code}</b></div>
                            <div><span style="color: #666; font-size: 12px;">客户价值分层</span><br><b>{distributor['客户价值分层']}</b></div>
                        </div>

                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 12px;">
                            <div><span style="color: #666; font-size: 12px;">所属区域</span><br><b>{district}</b></div>
                            <div><span style="color: #666; font-size: 12px;">省份</span><br><b>{province}</b></div>
                        </div>

                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 12px;">
                            <div><span style="color: #666; font-size: 12px;">销售总额</span><br><b>¥{distributor['销售总额']:,.2f}</b></div>
                            <div><span style="color: #666; font-size: 12px;">物料总成本</span><br><b>¥{distributor['物料总成本']:,.2f}</b></div>
                        </div>

                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 12px;">
                            <div><span style="color: #666; font-size: 12px;">ROI</span><br><b style="color: #0FC86F;">{distributor_roi:.2f}</b></div>
                            <div><span style="color: #666; font-size: 12px;">物料销售比率</span><br><b>{material_ratio:.2f}%</b></div>
                        </div>

                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 12px;">
                            <div><span style="color: #666; font-size: 12px;">物料多样性</span><br><b>{diversity} 种</b></div>
                            <div><span style="color: #666; font-size: 12px;">销售人员</span><br><b>{sales_person}</b></div>
                        </div>
        ...
        # ====================
        # 经销商物料使用时序分析 - Time Series Analysis of Distributor Material Usage
        # ====================
        st.markdown('<div class="feishu-chart-title" style="margin-top: 30px;">经销商物料使用时序分析</div>',
                    unsafe_allow_html=True)
        ...
        with col1:
            ...
            if len(filtered_distributor) > 0:
                ...
                fig.update_layout(
                    title=dict(
                        text=f"高效经销商物料投入与销售产出分析<br>({high_perf_name})",
                        font=dict(
                            size=15,
                            family="PingFang SC",
                            color="#1F1F1F"
        ...
        with col2:
            ...
            if len(filtered_distributor) > 0:
                ...
                fig.update_layout(
                    title=dict(
                        text=f"低效经销商物料投入与销售产出分析<br>({low_perf_name})",
                        font=dict(
                            size=15,
                            family="PingFang SC",
                            color="#1F1F1F"
