...
def create_distributor_analysis_tab(filtered_distributor, material_data, sales_data):
        ...
        if '客户价值分层' not in filtered_distributor.columns:
            ...
            if len(segment_counts) > 0:
                ...
                for segment in filtered_distributor['客户价值分层'].unique():
                    distributors = filtered_distributor[filtered_distributor['客户价值分层'] == segment]['经销商名称'].unique()
    ...
    if '物料多样性' in filtered_distributor.columns and 'ROI' in filtered_distributor.columns:
        ...
        if len(diversity_roi) > 0:
            ...
            # 在同一行右侧的区域显示材料使用偏好分析
            if '所属区域' in filtered_distributor.columns and len(material_data) > 0 and '物料类别' in material_data.columns:
                # 计算各区域各物料类别的使用情况
...
def main():
        ...
        # 修改标签页创建部分 - 只保留两个标签页
        tab1, tab2 = st.tabs(["物料与销售分析", "经销商分析"])

        # ======= 物料与销售分析标签页 =======
        with tab1:
            # 原来物料与销售分析标签页的内容，如create_improved_material_tab函数调用
