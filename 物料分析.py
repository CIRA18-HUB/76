...
def create_distributor_analysis_tab(filtered_distributor, material_data, sales_data):
    ...
    if missing_cols:
        ...
        # 计算物料多样性 - 如果数据中不存在
        if '物料多样性' not in matrix_data.columns and len(material_data) > 0 and '产品代码' in material_data.columns:
            # 检查是否有经销商名称列
            if '经销商名称' in material_data.columns:
                # 计算每个经销商使用的物料种类数
        ...
        fig.update_traces(
            ...
            customdata=np.column_stack((
                ...
                matrix_data['客户代码'],
                matrix_data['月份名'] if '月份名' in matrix_data.columns else ['未知'] * len(matrix_data)
            ))
        )

        # 安全确定数据范围
        if len(matrix_data) > 0:
            min_cost = matrix_data['物料总成本'].min()
            max_cost = matrix_data['物料总成本'].max()
    ...
    with col2:
        ...
        # 分析不同区域的物料使用偏好
        has_region_data = '所属区域' in filtered_distributor.columns
        has_material_data = len(material_data) > 0
        has_material_category = '物料类别' in material_data.columns if has_material_data else False

        if has_region_data and has_material_data and has_material_category:
            # 计算各区域各物料类别的使用情况
            ...
            if not has_region_data:
                missing_info.append("区域")
            if not has_material_data:
                missing_info.append("物料数据")
            elif not has_material_category:
                missing_info.append("物料类别")

            st.warning(f"数据中缺少{', '.join(missing_info)}信息，无法生成区域物料偏好分析。")
                ...
                cells=dict(
                    ...
                    fill_color=[
                        ...
                        [  # 高价值客户占比 - 颜色渐变
                            ...
                            for x in final_region_data['高价值客户'] if '高价值客户' in final_region_data.columns else [
                                                                                                                           'white'] * len(
                final_region_data)
            ],
            [  # 成长型客户占比 - 颜色渐变
                'rgba(43, 90, 237, ' + str(min(x / 50, 1)) + ')'
                for x in final_region_data['成长型客户'] if '成长型客户' in final_region_data.columns else [
                                                                                                               'white'] * len(
                final_region_data)
            ],
            [  # 稳定型客户占比 - 颜色渐变
                'rgba(255, 170, 0, ' + str(min(x / 50, 1)) + ')'
                for x in final_region_data['稳定型客户'] if '稳定型客户' in final_region_data.columns else [
                                                                                                               'white'] * len(
                final_region_data)
            ],
            [  # 低效型客户占比 - 颜色渐变
                'rgba(245, 63, 63, ' + str(min(x / 50, 1)) + ')'
                for x in final_region_data['低效型客户'] if '低效型客户' in final_region_data.columns else [
                                                                                                               'white'] * len(
                final_region_data)
            ],
            [  # 优化建议 - 按类别着色
