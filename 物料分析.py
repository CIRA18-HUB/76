...
def create_material_roi_analysis(filtered_material, filtered_sales):
    ...
    for idx, row in material_analysis.iterrows():
        category = str(row['物料类别'])
        for cat, factor in material_categories.items():
            if cat in category:
                material_analysis.at[idx, '效率因子'] = factor
                break
    ...
    # 计算各物料的平均产出比
    material_roi = material_analysis.groupby(['产品代码', '产品名称', '物料类别'])['物料产出比'].mean().reset_index()
