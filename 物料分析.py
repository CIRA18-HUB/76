...
def create_expense_ratio_analysis(filtered_distributor):
    ...
    # 第一个图表：TOP 15经销商物料费比
    st.markdown(
        '<div class="feishu-chart-container" style="box-shadow: 0 6px 16px rgba(0, 0, 0, 0.08); margin-bottom: 25px;">',
        unsafe_allow_html=True)
        ...
        if len(distributor_cost_ratio) > 0:
            ...
            fig.update_layout(
                ...
                yaxis=dict(
                    ...
                    ticktext=[f"{name[:15]}..." if len(name) > 18 else name for name in
