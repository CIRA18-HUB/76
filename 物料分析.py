...
def main():
        ...
        with tab1:
            ...
            if '物料类别' in filtered_material.columns and len(filtered_material) > 0:
                ...
                # 将英文月份替换为中文月份
                if pd.api.types.is_datetime64_dtype(pd.to_datetime(efficiency_matrix['月份名'])):
                    month_labels = pd.to_datetime(efficiency_matrix['月份名'].unique()).strftime('%Y年%m月')
                    fig.update_xaxes(
                        tickvals=efficiency_matrix['月份名'].unique(),
                ...
                fig.update_layout(
                    xaxis_title="月份",
                    yaxis_title="物料类别",
                    margin=dict(l=20, r=20, t=50, b=40),  # 增加底部和左侧边距避免遮挡
            ...
            with col1:
                ...
                if '销售人员' in filtered_distributor.columns and len(filtered_distributor) > 0:
                    ...
                    # 尝试从原始数据查找销售人员
                    unique_salespersons = []
                    if '销售人员' in material_data.columns:
                    ...
                    if unique_salespersons:
                        st.warning(
                            f"由于数据结构原因，无法正确显示销售人员效率分析。检测到系统中有销售人员数据：{', '.join(unique_salespersons[:5])}等{len(unique_salespersons)}人。请检查过滤条件或联系技术支持。")
                    else:
                        st.info("未检测到销售人员数据，请检查数据源。")
            ...
            with col2:
                ...
                # 计算销售人员的经销商客户结构
                if '客户价值分层' in filtered_distributor.columns and '销售人员' in filtered_distributor.columns:
                    # 计算每个销售人员的客户结构
                    ...
                    if len(salesperson_segments) > 0:
                        ...
                        # 只保留TOP销售人员的数据
                        if 'top_salespeople' in locals() and len(top_salespeople) > 0:
                        ...
                        fig.update_layout(
                            ...
                            xaxis_title='',
                            yaxis_title='经销商数量',
                            margin=dict(l=20, r=20, t=40, b=60),  # 增加底部边距避免遮挡
                            paper_bgcolor='white',
                            plot_bgcolor='white',
                            font=dict(
                                family="PingFang SC, Helvetica Neue, Arial, sans-serif",
                        ...
                        st.info("销售人员客户结构数据处理正常，但未能生成有效图表。请检查数据完整性。")
                else:
                    st.info("暂无客户分层或销售人员数据，无法生成客户结构分析。")
            ...
            # 确保有足够的数据进行季节性分析
            if len(material_data) > 0 and len(sales_data) > 0:
                # 计算每个月的物料成本和销售额
                ...
                # 判断是否有足够的数据进行季度分析
                if len(monthly_data) >= 3:  # 至少需要3个月才能进行季度分析
                    # 将月份转为季度
                    ...
                    quarterly_data['销售额占比'] = (
                                quarterly_data['销售金额'] / quarterly_data['销售金额'].sum() * 100).round(2)
                    quarterly_data['占比差异'] = (quarterly_data['销售额占比'] - quarterly_data['物料成本占比']).round(
                        2)
                    ...
                    st.info("数据量不足，无法进行季节性分析。需要至少3个月的数据才能进行季度分析。")
            else:
                st.info("缺少物料成本或销售额数据，无法进行季节性分析。")
